"""Team Rocket's Honchkrow — research-driven heuristic pilot (v24 = v14 + Fighting tempo override).

V24 CHANGE vs v14 (data-driven; tools/loss_probe.py showed all-prizes is 38% of our losses
vs Mega Lucario and v23's body-suppression cost us the race):
  vs a Fighting deck past the opening (turn>=4) we are RACING, not hoarding. In tempo
  mode we spend Rocket Feathers ammo to TAKE prizes now:
    T1  allow a NON-lethal Rocket Feathers at an HP>180 wall if it lands >=240 (a real
        2HKO setup: RF 240 + Hammer In 100 closes a 340 mega) instead of skipping it.
    T2  zero the RF ammo-preservation penalty so we fire RF-KOs on small/basic targets
        to bank cheap prizes.
    T3  reward a Rocket-Feathers KO (+60) in tempo mode — spend ammo to stay even/ahead
        in the prize race.
  Inert in non-Fighting matchups and in the opening (guard: abomasnow_cal).
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                if opp is None:
                    continue
                # v24 TEMPO OVERRIDE: vs a Fighting deck past the opening (turn>=4), we are
                # racing, not hoarding. all-prizes is 38% of our losses vs Mega Lucario, so in
                # tempo mode we spend Rocket Feathers ammo to TAKE prizes now instead of
                # banking it for a late mega OHKO.
                tempo = opp_fighting and state.turn >= 4
                ...
                # v24 relaxation: in tempo mode, allow a non-lethal RF that lands a real 2HKO
                # setup (>=240 dmg, so RF + Hammer In 100 closes a 340 mega).
                _rf_dmg = rocket_feathers_damage(mine, opp, tr_ammo) if is_rf else 0
                if is_rf and opp.hp > 180 and _rf_dmg < opp.hp and not (tempo and _rf_dmg >= 240):
                    continue
                if is_rf:
                ...
                if kos:
                    if is_rc:
                        ...
                        score -= (0 if tempo else 8 * tr_ammo)   # v24: race, don't hoard, vs Fighting
                if is_rf:
                    score -= (0 if tempo else 4 * tr_ammo)
                # v24 TEMPO: vs Fighting, a Rocket-Feathers KO is a prize taken NOW — reward
                # spending the ammo to race instead of hoarding for a late mega OHKO.
                if tempo and kos and is_rf:
                    score += 60
