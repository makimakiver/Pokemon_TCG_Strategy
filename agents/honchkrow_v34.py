...
def agent(obs_dict):
    ...
    # v34 MULTI-DOOR TERMINAL CHECK (planner spec): at prize 5 vs Fighting, try the CHEAPEST
    # winning finisher without pre-committing resources earlier. Three doors, cheapest first:
    #   Door 1 R-Command sacrifice: a ready (3-energy) Porygon2 fires R Command off the DISCARD
    #     pile (0 hand cost). Lethal if rc_fuel >= ceil(target_hp/20). Survival irrelevant (prize 6 ends game).
    #   Door 2 Split-attacker 2HKO: RF(x) this turn leaves the active mega at <=100hp AND a DCC
    #     Honchkrow exists for the Hammer-In follow-up. Allows a non-lethal RF chip that sets up the close.
    #   Door 3 Single OHKO: RF(x6)=360, the existing v26 path (handled by wall-doctrine when lethal).
    # If none feasible this turn, fall through to v26 behavior (no hoarding).
    door1_attack = door1_attacker = -1
    door2_attack = door2_attacker = -1
    prize5 = opp_fighting and op_prize_left == 1          # we have taken 5 prizes
    if prize5 and op_active is not None and op_prize_left <= prize_count(op_active):
        target_hp = op_active.hp
        # Door 1: scan Porygon2s in play for a ready one whose R-Command is lethal
        if PORYGON2 and R_COMMAND:
            need_fuel = (target_hp + RC_PER_CARD - 1) // RC_PER_CARD   # 20 per supporter
            if rc_fuel >= need_fuel:
                my_cards_full = [my_state.active[0] if my_state.active else None] + list(my_state.bench)
                for di, body in enumerate(my_cards_full):
                    if body is not None and body.id == PORYGON2:
                        _cd = card_table.get(PORYGON2)
                        if can_pay(list(body.energies), attack_table.get(R_COMMAND).energies or []):
                            door1_attacker, door1_attack = di, R_COMMAND
                            break
        # Door 2: RF chip into Hammer-In range + a DCC Honchkrow exists for the follow-up
        if door1_attack < 0 and HONCHKROW and RF_ATTACK:
            per = RF_PER_CARD * (2 if card_table.get(op_active.id) and
                                 card_table[op_active.id].weakness == EnergyType.DARKNESS else 1)
            # find the min RF-spend that leaves mega at <=100 (Hammer-In range)
            hk_dcc_ready = any(
                b is not None and b.id == HONCHKROW and len(b.energies) >= 3
                for b in (list(my_state.active) + list(my_state.bench)))
            if hk_dcc_ready:
                for spend in range(1, tr_ammo + 1):
                    if target_hp - per * spend <= 100:
                        door2_attacker, door2_attack = 0, RF_ATTACK   # active Honchkrow chips
                        break
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                # v34 MULTI-DOOR: at prize 5, prefer the cheapest winning door over the default plan.
                # Door 1 (R-Command, 0 hand cost) tops everything if feasible + lethal.
                if prize5 and mine.id == PORYGON2 and aid == R_COMMAND and kos \
                        and door1_attack == R_COMMAND:
                    score = 60000
                # Door 2: a non-lethal RF chip that sets up the Hammer-In 2HKO is allowed at prize 5.
                elif prize5 and is_rf and door2_attack == RF_ATTACK and not kos:
                    # only allow if it leaves the mega in Hammer-In range (the door check guaranteed it)
                    if plan.remain_hp <= 100 or (opp.hp - dmg) <= 100:
                        score = max(score, 40000)
                # Door 3 is the existing lethal RF (wall doctrine passes it when lethal) — handled normally.
