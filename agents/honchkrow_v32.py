...
def agent(obs_dict):
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if len(pokemon.energies) >= need:
            score -= 220
        # v32 CLOSER SURVIVAL: the race-audit showed in 47% of race-losses we had >=5 RF
        # ammo but NO DCC-ready (3-energy) Honchkrow alive to fire the mega OHKO — the
        # closer died short of 3 energy. So once we're racing (preserve/closer), keep
        # feeding a BENCHED Honchkrow to 3 energy as a dedicated closer reserve (it sits
        # safe on the bench, ready to promote + OHKO when the active dies).
        _is_bench_hk = (pokemon.id == HONCHKROW and is_active is False)
        racing = opp_fighting and (6 - op_prize_left) >= 3
        if racing and _is_bench_hk and len(pokemon.energies) < 3:
            score = max(score, 8400 + len(pokemon.energies) * 60)
        return score
