...
CONKELDURR, KYUREM = 115, 144            # v3: Seek Inspiration copy targets (250 / spread)
CIPHERMANIAC = 1188                       # v3: top-card setup (puts chosen cards on top of deck)
...
def agent(obs_dict):
    ...
    if pre_turn != state.turn:
        ...
        turn_actions = 0
    turn_actions += 1
    stalling = turn_actions > 60     # v3 stall fix: cap 80->60, breaks the 29-draw mirror loop
    ...
    deck_count_var = getattr(my_state, 'deckCount', 99)             # v3: copy-engine deck probe
    slowking_in_play = any(c is not None and c.id == SLOWKING
                           for c in (list(my_state.active) + list(my_state.bench)))
    conk_in_deck_proxy = (field_counts.get(CONKELDURR, 0) == 0) and (deck_count_var > 4)
                ...
                if not can_pay(attached_types, cost):
                    if have_spare_energy and not state.energyAttached and DECK_ETYPES and \
                       any(can_pay(attached_types + [et], cost) for et in DECK_ETYPES):
                        ...
                        continue
                is_seek = (mine.id == SLOWKING and getattr(atk, 'name', '') == 'Seek Inspiration')
                for j, opp in enumerate(op_cards):
                    if opp is None or j != 0:
                        continue
                    dmg = attack_damage(mine, atk, opp)
                    # v3 COPY ENGINE: Seek Inspiration (printed dmg=0) copies the top card's
                    # attack. We can't see the top, but if a copy target is still in deck, score
                    # Seek as that target's damage so the pilot actually FIRES it (v2 never did).
                    # Best target: Conkeldurr Gutsy Swing 250 (vs active); Kyurem Trifrost 110x3
                    # spread (vs active, conservative). Conkeldurr available if not all copies are
                    # in play/discard. Use deckCount as a proxy the combo is settable.
                    if is_seek and dmg <= 0:
                        conk_in_deck = (field_counts.get(CONKELDURR, 0) == 0) and \
                                       (deck_count_var > 4)  # deck not empty; Conkeldurr plausibly in deck
                        if conk_in_deck:
                            dmg = 250                       # Conkeldurr Gutsy Swing (pay Seek's [P,C])
                    if dmg <= 0:
                        continue
                    ...
                    if mine.id == SLOWKING:               # v2 lever 2: prefer the 1-prize identity attacker
                        score += 120
                    # v3 COPY ENGINE: a Seek Inspiration that lands a 250-dmg Conkeldurr copy is
                    # the deck's best line. Strongly prefer it so it wins the plan decision.
                    if is_seek and dmg >= 200:
                        score += 600
                    if score > best:
                        best = score
                        plan.attacker = i
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    score = 3000
                    # v3 CIPHERMANIAC PRIORITY: it puts Conkeldurr on top -> sets up the 250 copy.
                    # Boost hard when Slowking is up and Conkeldurr is still in deck.
                    if card.id == CIPHERMANIAC and slowking_in_play and conk_in_deck_proxy:
                        score = 9000
            ...
            if o.attackId == plan.attack_id:
                score += 500
            # v3 STALL FIX: when churning (stalling flag), prefer ANY legal attack over END
            # to break the mirror loop that produced 29 draws in v2.
            if stalling:
