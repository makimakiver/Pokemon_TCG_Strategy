...
os.environ.setdefault("BARE_DECK", os.path.join(
    ...
    "data", "decks", "deck_starmie_cind_v10.json"))
...
POFFIN = 1086
SALVATORE = 1189     # Supporter: evolves a no-Ability Stage-1 from a just-benched Basic THIS TURN (Rare Candy
                      # can't evolve just-played basics). v8 force-play to land Starmie ex a turn early (race tempo).
BUDEW = 235          # 30hp basic. Itchy Pollen (atk 323): opp can't play Items next turn. v10 item-lock vs item-reliant decks.
ITCHY_POLLEN = 323
HEROS_CAPE = 1159       # +100HP tool. v4 force-attach to the ACTIVE basic early to survive the OHKO
                        # window (traced: 70hp Staryu OHKO'd -> no-active wipe vs megalucario/hydrapple/crustle).
...
def agent(obs_dict):
    ...
    # v10 BUDEW ITEM-LOCK — if Budew is active and can attack, force Itchy Pollen to lock the opponent's Items
    # next turn. Megalucario is item-reliant (Dusk Ball/Premium Power Pro/Fighting Gong/Poke Pad) -> stalls their
    # setup, buys Starmie the evolve window. Budew is 30hp (sacrificial), but the lock persists into opp's next turn.
    if active is not None and active.id == BUDEW:
        for i, o in enumerate(opts):
            if o.type == OptionType.ATTACK and getattr(o, "attackId", None) == ITCHY_POLLEN:
                _log(turn, "Itchy Pollen (item-lock)", "Budew active -> lock opponent Items next turn (stall item-reliant setup)")
                return [i] if sel.minCount <= 1 else [i]

    # v8 SALVATORE TEMPO — force Salvatore to evolve a benched/active Staryu->Starmie ex same-turn (T1-T3) when
    # Starmie ex isn't in play yet. Lands the 330hp tank a turn early so it attacks first in the Lucario Brave/Jab
    # race. Salvatore works on a Staryu benched THIS turn (via Poffin) — Rare Candy can't. Narrow: T1-T3 + no Starmie ex yet.
    if turn <= 3 and not starmie_in_play and SALVATORE in hand and STARYU in in_play:
        for i, o in enumerate(opts):
            if o.type == OptionType.PLAY:
                c = get_card(obs, AreaType.HAND, o.index, st.yourIndex)
                if c is not None and c.id == SALVATORE:
                    _log(turn, "Salvatore (evolve tempo)", "Staryu in play + Salvatore -> land Starmie ex same-turn (race tempo vs Lucario)")
                    return [i] if sel.minCount <= 1 else [i]

    # v4 EARLY CAPE — attach the single Hero's Cape to the ACTIVE basic (Staryu 70->170hp, Cinderace
    # 160->260hp) early, to survive the OHKO window that causes no-active board-wipes (the traced root cause).
    # bare_agent attaches Cape late/to an evolved Starmie; we force it onto the early active basic T1-T3.
    # 170hp Staryu survives hydrapple/crustle hits (megalucario 270 still kills — structural).
    if turn <= 3 and active is not None and getattr(active, "id", 0) in (STARYU, CINDERACE) and HEROS_CAPE in hand:
        already = any(getattr(t, "id", None) == HEROS_CAPE for t in (getattr(active, "tools", []) or []))
        if not already:
            for i, o in enumerate(opts):
                if o.type == OptionType.ATTACH:
                    ...
                    if card is not None and card.id == HEROS_CAPE and getattr(o, "inPlayArea", None) == 4:
                        ...
                        if tgt is not None and tgt.id in (STARYU, CINDERACE):
                            _log(turn, "attach Hero's Cape (early)", f"active {tgt.id} basic + Cape -> +100HP to survive OHKO window")
                            return [i] if sel.minCount <= 1 else [i]
