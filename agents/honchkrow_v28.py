"""Team Rocket's Honchkrow — research-driven heuristic pilot (v28 = v26 + HAMMER-IN CLOSER).

V28 CHANGE vs v26 (data-driven; v27 R-Command closer FAILED because Porygon2 is Fighting-
WEAK + 90hp = free OHKO. And the race-loss endgame has HAND=0, so Rocket Feathers (which
scales on HAND supporters) does 0 damage — RF cannot be the closer once the hand is spent.
The deck's only HAND-INDEPENDENT closer that is NOT Fighting-weak is Honchkrow 'Hammer In'
(DCC, flat 100). Honchkrow has Fighting RESISTANCE (-30, survives Aura Jab) and Hammer In
100 KOs the opponent's basics (Riolu 80 / Makuhita 80). So in CLOSER MODE we set up + fire
Hammer In as the hand-free finisher instead of dying on an empty hand:
    H1  closer mode + low RF ammo (can't OHKO the mega) -> develop a Honchkrow to DCC (3
         energies) so Hammer In is online; remove the 'stop at 2 energy' penalty in closer.
    H2  closer mode: a Hammer In line that KOs gets a big bonus (+350) — it's the only
         hand-independent prize we can bank when RF is dead.
  Guardrail: Hammer In only chips 100 vs the 340 mega, so vs the mega we still need the RF
  OHKO — Hammer In is for picking off basics for the final prizes. (guard: abomasnow_cal)
...
def agent(obs_dict):
            ...
            for aid in (cd.attacks or []):
                ...
                if kos:
                    if is_rc:
                        ...
                        # v28: closer mode + can't OHKO mega with RF (low ammo) -> Hammer In
                        # KOs (basics) are our hand-independent finisher. Big boost.
                        if closer and tr_ammo < 6:
                            score += 350
                    elif is_rf:
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if len(pokemon.energies) >= need:
            score -= 220
        # v28 HAMMER-IN CLOSER: in closer mode, keep developing Honchkrow to DCC (3 energy)
        # so Hammer In is online as the hand-free finisher; waive the stop-at-2 penalty.
        if closer and pokemon.id == HONCHKROW and len(pokemon.energies) < 3 and tr_ammo < 6:
            score = max(score, 8200 + len(pokemon.energies) * 40)   # keep feeding the closer
        return score
