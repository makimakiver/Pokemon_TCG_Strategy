...
my_deck = ([463] * 4 + [891] * 3 + [473] * 2 + [474] * 1 + [414] * 2 +
           ...
           [1152] * 4 + [1134] * 4 + [1122] * 4 +   # Poke Pad, Transceiver, +4 Pokegear 3.0 (deep supporter tutor)
           [1077] * 2 + [1097] * 3 +               # Roto-Stick (trimmed 4->2), Night Stretcher x3
           [1257] * 3 + [15] * 4 + [17] * 4)        # Factory, TR Energy, Ignition (unchanged)
assert len(my_deck) == 60, f"deck has {len(my_deck)} cards"
# v39: v26 deck, bodies+energy+supporters INTACT, +4 Pokegear 3.0 (swap -2 Roto-Stick, -1 Miracle
# Headset, -1 Ultra Ball). Pokegear digs top-7 for a Supporter (vs Roto-Stick top-4) = strictly
# deeper consistency engine -> targets the Abomasnow slow-game gap without Lucario body/energy loss.
...
POKEGEAR = _card_named("Pokégear 3.0")          # v39: top-7 Supporter tutor (deep consistency)
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if deck_count <= 12:
                        ...
                        score = 3500
                    # v39 Pokegear 3.0: deepest supporter tutor (top-7 vs Roto-Stick top-4). When no
                    # draw/tutor Supporter is in hand, fire Pokegear to find the engine piece. This is
                    # the consistency lift that helped the slow (Abomasnow) matchup, applied to v26's shell.
                    if cid == POKEGEAR:
                        have_draw_supporter = any(hand_counts.get(s, 0) > 0 for s in (ARIANA, PETREL, GIOVANNI, PROTON))
                        if not have_draw_supporter:
                            score = max(score, 4800)
                        else:
                            score = max(score, 3800)   # still a good dig, just not urgent
