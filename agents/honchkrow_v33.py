"""Team Rocket's Honchkrow — research-driven heuristic pilot (v33 = v26 + MEGA-RESERVE gate).

V33 CHANGE vs v26 (planner spec, debate-narrowed): the failure mode of v26's race-losses is
reaching prize 5 then lacking the ammo to OHKO the mega because RF supporters were spent on
side-KOs getting there. v31/v32 regressed because they hoarded from early turns, starving the
mill engine that wins 76% of games. v33 gates ONLY the endgame (>=4 prizes taken), and only
blocks multi-supporter RF spends on NON-winning / NON-mega-KO side targets:
    MR  vs Fighting, >=4 prizes taken: do NOT spend >=2 RF supporters on a Rocket Feathers
        attack unless it (a) wins the game (prize 6) or (b) KOs the active Mega Lucario. For
        taking prize 5 vs a non-mega, prefer Hammer In / R Command / single-supporter RF.
  This leaves the mill engine untouched for the first ~3 prizes and reserves the mega-OHKO
  ammo for the close. (guard: abomasnow_cal)
...
def agent(obs_dict):
    ...
    # v33 MEGA-RESERVE gate: vs Fighting, once we've taken >=4 prizes, block multi-supporter
    # RF spends on NON-winning / NON-mega-KO side targets so the mega-OHKO ammo survives to
    # the close. We_took = 6 - op_prize_left. mega_up = the active target is Mega Lucario.
    we_took = 6 - op_prize_left
    mega_reserve = opp_fighting and we_took >= 4 and op_prize_left > 1
    mega_up = op_active is not None and op_active.id == 678
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                kos = opp.hp <= dmg
                # v33 MEGA-RESERVE: in the endgame (>=4 prizes), spending >=2 RF supporters on a
                # Rocket Feathers that neither wins (prize 6) nor KOs the active mega strands the
                # mega-OHKO ammo. Compute how many supporters THIS RF would need, then skip it.
                if is_rf and mega_reserve:
                    per = RF_PER_CARD
                    if getattr(card_table.get(opp.id), "weakness", None) == EnergyType.DARKNESS:
                        per *= 2
                    rf_cost = max(1, (opp.hp + per - 1) // per)
                    winning = kos and op_prize_left <= prize_count(opp)
                    if rf_cost >= 2 and not winning and not (kos and mega_up):
                        continue          # reserve the mega-OHKO ammo; take the side-prize another way
                if kos:
                    score += 500
