...
def agent(obs_dict):
    ...
    # 14. v8/v9 PROMOTE TRIGGER (MAIN) — if active is a weak basic (<=100hp, non-Dragapult) AND a benched
    #    Dragapult is ready (evolved + Fire+Psychic) → Switch-item or RETREAT to promote it.
    #    v9 PRIZE-ECONOMY GATE (planner): only promote when TIED-or-AHEAD on prizes. Promoting a 2-prize
    #    Dragapult ex while BEHIND gifts the opp's prize-racer (lucario/bellibolt) the KO that ends the
    #    game — the cause of v8's -8/-9 regressions. mogja_j promotes profitably because it's ahead.
    #    When behind, keep the 1-prize weak active as a sacrifice wall; develop Dragapult further.
    ...
    if active is not None and active.id != DRAGAPULT and getattr(active, "hp", 0) <= 100 \
            and my_prizes_taken >= opp_prizes_taken:   # tied or ahead only (v9 gate)
        def _ready2(c):
            es = set(int(e) for e in (c.energies or []))
        ...
        if ready_drag:
            ...
            if si is not None and sel.minCount <= 1:
                _log(turn, "Switch (→Dragapult)", f"active {card_name(active.id)} weak, {my_prizes_taken}-{opp_prizes_taken} tied/ahead → Switch in ready Dragapult")
                return [si]
            rcost = card_retreat(active.id)
            if len(active.energies) >= rcost:
                for i, o in enumerate(opts):
                    if o.type == OptionType.RETREAT:
                        _log(turn, "Retreat (→Dragapult)", f"active {card_name(active.id)} weak, {my_prizes_taken}-{opp_prizes_taken} tied/ahead → promote ready Dragapult")
                        return [i] if sel.minCount <= 1 else [i]
