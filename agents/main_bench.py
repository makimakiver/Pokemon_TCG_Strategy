...
def agent(obs_dict: dict) -> list[int]:
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # Going FIRST tested clearly better than second (62% vs 38% head-to-head).
            score = 1
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    # Build the board, but don't flood the bench with basics (cap 3 bodies
                    # is a tested local optimum: more or fewer both test worse in the mirror).
                    score = 20000
                    ...
                    # Play items BEFORE the hand-shuffle draw supporter (1227 Lillie's
                    # Determination shuffles your hand into the deck). Otherwise unplayed
                    # items (1235 energy accel, 1086 Pokemon search) get shuffled away.
                    score = 3500
                elif ct == CardType.SUPPORTER:
                    score = 3000  # draw/search engine — play after items, once per turn
