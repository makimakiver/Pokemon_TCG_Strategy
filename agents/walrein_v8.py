...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
        ...
        # v8 EARLY-TUTOR GATE: force setup tutors ONLY in the opening (turn<=4) before Walrein is
        # online — finds the Walrein line + RC fast (alakazam +10 in v7) without late-game over-draw
        # (which hurt crustle/hydrapple in v7).
        if not walrein_in_play and (state.turn or 0) <= 4:
            opts = select.option or []
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    c = get_card(obs, AreaType.HAND, o.index, my_index)
                    if c is not None and c.id in (1086, 1081, 1192):
                        return [i] if select.minCount <= 1 else [i]
