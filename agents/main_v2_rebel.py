"""main_v2_rebel: self-contained anti-Crustle "Rebel" Fire-aggro agent.

A hard counter to the mono-Grass Crustle deck (`main.py`). Crustle's whole line is
weak to Fire, it runs only 8 Pokemon, and its loss mode is getting KO'd off its thin
board. This deck is fast NON-ex Fire aggro:
...
going-second coin flip is what holds the overall just under a stable 80%). Deck is inlined
below so the module is self-contained; BARE_DECK env still overrides for experiments.
"""
import json
import os
...
# ---- Rebel Fire deck, inlined (single-file submission like the other main_* agents) ----
my_deck = (
...
_override = os.environ.get("BARE_DECK", "")
if _override and os.path.exists(_override):
    my_deck = json.load(open(_override))
