"""Walrein/Spheal — v3 = bare_agent + FANGS-STALL (data-driven: win path is deck-out/attrition) (Frigid Fangs is the win condition).
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                for j, opp in enumerate(op_cards):
                    ...
                    # v3 FANGS-STALL: the win path is deck-out/attrition (zoroark 60/91 deck-out
                    # wins). Maintain the Frigid Fangs lock (prevent opp attacks -> they deck out or
                    # collapse). AVOID Megaton self-damage unless it wins-now or KOs a 2-prizer
                    # (self-50 kills Walrein and breaks the stall — v2's failure).
                    is_fangs = (mine.id == WALREIN and aid == FRIGID_FANGS)
                    is_megaton = (mine.id == WALREIN and aid == MEGATON_FALL)
                    winning_ko = (opp.hp <= dmg and op_prize_left <= prize_count(opp))
                    if is_fangs:
                        # Fangs maintains the lock + chips 60 (no self-damage). PREFER it to stall.
                        op_en = len(opp.energies) if hasattr(opp, 'energies') else 0
                        if op_en <= 2:
                            score += 600          # lock holds -> preserve it, chip, survive
                    if is_megaton:
                        # Megaton = 170 but +50 self -> only worth it for a KO/win, else it breaks stall.
                        if not (winning_ko or (opp.hp <= dmg and prize_count(opp) >= 2)):
                            score -= 700          # avoid self-damage that breaks the stall
                        walrein_hp = mine.hp if hasattr(mine, 'hp') else 170
                        if walrein_hp <= 60:
                            score -= 600          # never self-KO Walrein
                    if i == 0:
                        score += 200
