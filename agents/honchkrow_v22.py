...
def agent(obs_dict):
    ...
    # v21 (codex rule): snowman_mode if the opponent has shown the Abomasnow/Kyogre Water box.
    # In snowman_mode, Rocket Feathers ammo is ONE 7-card combo reserved for the Mega OHKO —
    # never spend it on Snover/Kyogre side-KOs (use Hammer In / R Command for those).
    _SNOWMAN_IDS = {723, 722, 418, 419, 721}   # Mega Abomasnow ex / Snover / Abomasnow / Kyogre
    MEGA_ABOM = 723
    snowman_mode = any(c is not None and c.id in _SNOWMAN_IDS
                       for c in ([op_active] + list(op_state.bench)))
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                if is_rf and opp.hp > 180 and rocket_feathers_damage(mine, opp, tr_ammo) < opp.hp:
                    continue
                # v21 (codex): in snowman_mode, Rocket Feathers fires ONLY for the decisive
                # Mega Abomasnow OHKO (≥7 ammo to clear Frost Barrier's -30) or a game-ending
                # prize. Otherwise preserve every supporter — take side KOs with Hammer In /
                # R Command instead.
                if is_rf and snowman_mode:
                    final_prize = (opp.hp <= rocket_feathers_damage(mine, opp, tr_ammo)
                                   and op_prize_left <= prize_count(opp))
                    mega_ohko = (opp.id == MEGA_ABOM and tr_ammo >= 7)
                    if not (final_prize or mega_ohko):
                        continue
                if is_rf:
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if cid in BASIC_MONS and n_bodies >= 5:
                        score = 30
                    # v22 (codex "bench only attackers you can power"): Articuno cannot attack
                    # in this list (Dark Frost needs Water we don't run) -> never bench it.
                    if cid == ARTICUNO:
                        score = 25 if n_bodies <= 1 else 5
                    ...
                    if cid == GIOVANNI:
                        ...
                        if deck_count <= 8:
                            ...
                            score = 1200
                        # v22 (codex "max energy consistency"): vs the snowman we lose to the
                        # race, not deck-out, and energy is the dominant lever (MC: p_energy
                        # 0.55->0.85 = 29%->58%). Dig hard for the untutorable energy when our
                        # attacker is starved. Snowman-gated so the common Lucario matchup keeps
                        # its deck-out discipline.
                        if snowman_mode and active_starved and deck_count >= 12:
                            score = 6000
                    elif cid == ARCHER:
