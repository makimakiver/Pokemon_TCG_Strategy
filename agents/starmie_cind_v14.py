...
os.environ.setdefault("BARE_DECK", os.path.join(
    ...
    "data", "decks", "deck_starmie_cind_v4.json"))
...
def agent(obs_dict):
    ...
    # v14 OPENING ACTIVE — force the highest-HP basic as the opening active (SETUP_ACTIVE_POKEMON, context 1).
    # bare_agent scores ALL basics equally (100) => picks arbitrarily, often the 70hp Staryu (dies T2 -> wipe). Override
    # to the tanky opener (Cinderace 160 > Staryu 70) so the active survives Lucario's early Aura Jab(130)/chip.
    if sel.context == 1:
        best_i = None; best_hp = -1
        for i, o in enumerate(sel.option or []):
            if o.type in (OptionType.CARD, OptionType.PLAY):
                c = get_card(obs, AreaType.HAND, getattr(o, "index", 0), st.yourIndex)
                if c is not None:
                    cd = B.card_table.get(c.id)
                    hp = getattr(cd, "hp", 0) if cd else 0
                    if hp > best_hp:
                        best_hp = hp; best_i = i
        if best_i is not None:
            _log(0, "setup-active (high-HP)", f"force {best_hp}hp basic as opening active (survive early wipe)")
