...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
        ...
        # v7 SETUP-TUTOR PRIORITY: when Walrein isn't online yet, force the draw/tutor supporters
        # (Colress 1086 / Irida 1081 / Carmine 1192) to find the Walrein line + Rare Candy faster.
        if not walrein_in_play:
            opts = select.option or []
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    c = get_card(obs, AreaType.HAND, o.index, my_index)
