...
def agent(obs_dict):
    ...
    # v31 CLOSER PRESERVATION (planner counterfactual pushback): start reserving RF ammo
    # EARLIER (from prize 3 onward) so the mega OHKO is reachable in the endgame, and the
    # terminal empty-hand state is avoided. `we_took` = prizes we've banked.
    we_took = 6 - op_prize_left
    preserve = opp_fighting and we_took >= 3 and op_prize_left > 2 and len(my_state.prize) > 2
    # is the mega the active target right now? (the closer line is only 'live' if the mega
    # is up or promotable; if not, we're not reserving for it yet)
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                if is_rf:
                    score -= 4 * tr_ammo
                # v31 CLOSER PRESERVATION: from prize 3 onward, REFUSE a non-winning Rocket
                # Feathers side-KO once the mega closer line is live — burning 1-3 supporters on
                # a small target strands us short of the 6-ammo mega OHKO in the endgame (the
                # empirically-observed terminal empty-hand state). Only spend RF if it KOs the
                # mega this turn OR no mega is in play to close on.
                if is_rf and preserve and mega_up and not kos:
                    score -= 2000
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if cid in ATTACKERS:
                        ...
                        score = 200          # ammo / fuel
                        # v31: in preserve mode, RF supporters are reserved closer ammo —
                        # value grabbing them HIGH so they stay in hand for the mega OHKO.
                        if preserve and mega_up:
                            score = 500
                    elif cid == FACTORY:
                    ...
                    if cid in ATTACKERS or cid in BASIC_MONS:
                        ...
                        # v31: in preserve mode, refusing to discard RF ammo keeps the mega
                        # OHKO reachable — only pitch if it's an extra (>=2 in hand).
                        if preserve and mega_up and hand_counts.get(cid, 0) < 2:
                            score = -500
                    elif cid in ALL_ENERGY:
