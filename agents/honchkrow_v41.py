"""Team Rocket's Honchkrow — research-driven heuristic pilot (v26 = v25 + CLOSER MODE).
...
my_deck = ([463] * 4 + [891] * 3 + [473] * 2 + [474] * 1 + [414] * 2 +
           [434] * 1 + [272] * 1 +               # Mimikyu (F-resistant wall), Lillie's Clefairy ex (closer)
           [1220] * 4 + [1216] * 4 + [1218] * 4 + [1219] * 4 + [1217] * 4 +
           [1134] * 4 + [1152] * 4 + [1077] * 3 + [1097] * 2 +
           [1109] * 1 + [1174] * 1 +               # Miracle Headset, Air Balloon (retreat -CC)
           [1257] * 3 + [15] * 4 + [17] * 2 + [5] * 2)    # Factory, TR Energy(4), Ignition(2), basic Psychic(2, for Lillie [P,C])
assert len(my_deck) == 60, f"deck has {len(my_deck)} cards"
# deck = user list (14P/38T/8E) with ENERGY FIX (-2 Ignition +2 basic Psychic id 5):
#       basic Psychic attaches to Lillie (TR Energy cannot, non-TR) -> Full Moon Rondo [P,C] payable. 4 Murkrow/3 Honchkrow/2 Porygon/1 Porygon2/2 Articuno/1 Mimikyu/
# 1 Lillie's Clefairy ex; 4 each Giovanni/Proton/Petrel/Ariana/Archer/Transceiver/Poke Pad, 3 Roto-
# Stick, 2 Night Stretcher, 1 Miracle Headset, 1 Air Balloon, 3 Factory; 4 TR Energy/4 Ignition.
...
MIMIKYU = _card_named("Team Rocket's Mimikyu")
LILLIE = _card_named("Lillie’s Clefairy ex")        # v40: 190hp Metal-weak closer (Full Moon Rondo)
...
FULL_MOON = _attack_named(LILLIE, "Full Moon Rondo")      # v40: [P,C], 20 + 20×(total benched)
...
# v40 instrumentation: is the Lillie closer actually reachable in-engine? TR Energy text says
# "Team Rocket's Pokémon only" and Lillie is NOT TR -> TR Energy may be unable to attach to it,
# making Full Moon Rondo [P,C] unpayable. Measure the truth, don't assume.
_v40 = {"lillie_benched_games": 0, "lillie_energy_attaches": 0,
        "lillie_reached_2energy": 0, "fullmoon_fired": 0, "fullmoon_kills": 0}
...
def agent(obs_dict):
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                is_fullmoon = (mine.id == LILLIE and aid == FULL_MOON)   # v40 Lillie closer
                ...
                if is_rf:
                    ...
                    # v40 Full Moon Rondo: 20 + 20×(total benched, BOTH players). Apply weakness
                    # (Fairy Zone makes opp {N} weak to Psychic; Mega Lucario is already Psychic-weak).
                    total_benched = sum(1 for c in my_state.bench if c is not None) + \
                                    sum(1 for c in op_state.bench if c is not None)
                    _fm = 20 + 20 * total_benched
                    _tdata = card_table.get(opp.id)
                    _wkn = getattr(_tdata, "weakness", None) if _tdata else None
                    dmg = _fm * (2 if _wkn == EnergyType.PSYCHIC else 1)
                    _v40["fullmoon_fired"] += 1
                else:
                ...
                if kos:
                    if is_rc:
                        ...
                        _v40["fullmoon_kills"] += 1
                # v40 LILLIE CLOSER: Full Moon Rondo is the deck's best closer (survives 190hp,
                # scales with bench, no hand-cost). When payable + lethal, strongly prefer it.
                if is_fullmoon:
                    score += 1200
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if pokemon.id == HONCHKROW:
            ...
            score += 380
        elif pokemon.id == LILLIE:
            # v40 CLOSER-DEVELOP: Lillie needs [P,C] for Full Moon Rondo. TR Energy text says
            # "Team Rocket's Pokémon only" (Lillie isn't TR) so it may be unattachable -> the
            # instrumentation will reveal the truth. If attachable, boost hard to develop it.
            # Only invest once benched + the kill window is measurable (total benched >=6).
            total_benched_now = sum(1 for c in my_state.bench if c is not None) + \
                              sum(1 for c in op_state.bench if c is not None)
            if not is_active and total_benched_now >= 6 and len(pokemon.energies) < 2:
                score = 9000 + len(pokemon.energies) * 80   # build the closer
            else:
                score += 60   # modest wall value when not developing
        ...
        if len(pokemon.energies) >= need:
            score -= 220
        if pokemon.id == LILLIE and len(pokemon.energies) < 2 and not is_active:
            # track Lillie energy accrual for instrumentation (is the closer reachable?)
            if len(pokemon.energies) == 1:
                _v40["lillie_reached_2energy"] = 1
        return score
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if card.id == MURKROW:
                        ...
                        # v40 SAFETY GATE (planner): Lillie is a 2-prize ex. Don't bench it early
                        # vs Fighting (gust-isolate risk). Bench once board>=4 OR the kill window is
                        # near (>=6 total benched). Hold in hand otherwise.
                        total_benched_now = sum(1 for c in my_state.bench if c is not None) + \
                                          sum(1 for c in op_state.bench if c is not None)
                        safe_to_bench = (n_bodies >= 4) or (total_benched_now >= 5)
                        if safe_to_bench:
                            score = 115; _v40["lillie_benched_games"] = 1
                        else:
                            score = 5     # hold in hand until safe
                    elif card.id == MIMIKYU:
                        score = 60 if opp_fighting else 105
            ...
            if card is not None and card.id in TOOLS:
                ...
                # v40 Air Balloon (1174): retreat -CC. Attach to active Honchkrow so we can free-
                # swap to Lillie for the Full Moon Rondo close. Skip on Lillie (it's ex; Balloon
                # is for mobility of the active attacker).
                if card.id == 1174:
                    if pokemon is not None and pokemon.id == HONCHKROW and o.inPlayArea == AreaType.ACTIVE:
                        score += 1500
                    elif pokemon is not None and pokemon.id == LILLIE:
                        score = 50
            else:
                ...
                # v40 instrumentation: count energy attaches routed to Lillie
                if pokemon is not None and pokemon.id == LILLIE:
                    _v40["lillie_energy_attaches"] += 1
