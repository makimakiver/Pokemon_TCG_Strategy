            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if cid in BASIC_MONS and n_bodies >= 5:
                        score = 30
                    # v17: Articuno's only attack (Dark Frost, WCC) needs Water energy this
                    # deck CANNOT provide -> it can never attack. Benching it just gifts the
                    # opponent a free prize on a dead body. Only play it as a last-resort body
                    # (e.g. forced / empty board), never proactively.
                    if cid == ARTICUNO:
                    ...
                    if cid == GIOVANNI:
                        ...
                        if deck_count <= 8:
                            ...
                            score = 1200
                        # v15: this deck CANNOT tutor energy (8 cards, draw-luck only). When
                        # the attacker is starved AND the deck is deep (no deck-out risk), dig
                        # HARD for energy — vs fast aggro (Mega Lucario / Abomasnow) energy
                        # screw, not deck-out, is what loses us the game.
                        if active_starved and deck_count >= 22:
                            score = 6000
                    elif cid == ARCHER:
