"""Generic "bare setup" agent: pilots ANY deck with no archetype-specific tuning.
...
# Dwebble->Crustle (Grass, 120) + Cyndaquil->Quilava->Typhlosion (Fire, St2 170hp/160 dmg),
# Rare Candy (skip-evolve), Ultra Ball (search any), Boss's Orders (gust). 20 Pokemon / 16
# energy -- far more consistent than the single-line Palace list (8 Pokemon / 31 energy).
my_deck = (
...
# ---- ONLY overlay: Buddy Blast real-damage awareness (no preference bonuses) ----
# Ethan's Typhlosion (354) "Buddy Blast" reads as base 40 in the card table but truly
# deals 40 + 60 per Ethan's Adventure (1215) in our discard. The generic engine can't see
# this, so it never values the attack. We correct ONLY the damage; attacker choice still
# flows through normal scoring.
ETHANS_ADVENTURE = 1215
_BUDDY_BLAST_AID = None
_t = card_table.get(354)
if _t and getattr(_t, "attacks", None):
    for _aid in _t.attacks:
        _a = attack_table.get(_aid)
        if _a is not None and (getattr(_a, "damage", 0) or 0) == 40:
...
def attack_damage(attacker, atk, target, ethan_fuel=0):
    ...
    # Buddy Blast: real damage is 40 + 60 per Ethan's Adventure in our discard.
    if _BUDDY_BLAST_AID is not None and getattr(atk, "attackId", None) == _BUDDY_BLAST_AID:
        dmg = 40 + 60 * ethan_fuel
    if dmg <= 0:
        return 0
...
def agent(obs_dict):
    ...
    if context == SelectContext.MAIN:
        ...
        ethan_fuel = sum(1 for c in (my_state.discard or []) if c.id == ETHANS_ADVENTURE)
