...
os.environ.setdefault("BARE_DECK", os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "decks", "deck_dragapult_v12.json"))
...
HEROS_CAPE = 1159                # Hero's Cape (+100HP tool) — mogja_j's T1 move; flips the mirror race
LEGACY_ENERGY = 12             # Legacy Energy (any-type + -1 prize when bearer KO'd) — fixes the 2-prize liability
...
def agent(obs_dict):
    ...
    # ---- v8 lever 13: PROMOTE a benched evolved+ready Dragapult to active (the behavioral fix).
    # Fires in the SWITCH/TO_ACTIVE select after RETREAT/Switch-item. Planner-refined gate: Dragapult
    # energy>=2 AND color-complete (Fire+Psychic for Phantom Dive). Core bug fix — the data shows
    # Dragapult sits benched with >=2 energy 74x while Budew attacks for 10 dmg.
    if sel.context in (3, 4):   # SelectContext.SWITCH=3, TO_ACTIVE=4
        def _ready(c):
            if c.id != DRAGAPULT: return False
            es = set(int(e) for e in (c.energies or []))
            return len(c.energies) >= 2 and 2 in es and 5 in es   # Fire(2) + Psychic(5)
        ready_drag = [c for c in (me.bench or []) if c is not None and _ready(c)]
        if ready_drag:
            for i, o in enumerate(sel.option or []):
                if o.type in (OptionType.CARD, OptionType.PLAY):
                    ...
                    if oarea == 5 and opidx is not None:   # AreaType.BENCH == 5
                        ...
                        if bc is not None and bc.id == DRAGAPULT:
                            _log(st.turn, "promote Dragapult", "ready (Fire+Psychic) Dragapult on bench → make it active")
    ...
    # 0.5 v11 HERO'S CAPE — force-attach the +100HP tool to the active Dragapult (mogja_j's T1 move).
    #    Dragapult 320->420hp: in the MIRROR it survives 3 Phantom Dives vs the opp's 2 (asymmetry wins
    #    the race); everywhere it buffers chip/spread. bare_agent undervalues tools on tanks.
    if active_is_drag and HEROS_CAPE in hand:
        # already tooled? check active's tools
                ...
                if o.type == OptionType.ATTACH:
                    ...
                    if card is not None and card.id == HEROS_CAPE and getattr(o, "inPlayArea", None) == 4:
                        ...
                        if tgt is not None and tgt.id == DRAGAPULT:
                            _log(turn, "attach Hero's Cape", "Dragapult active + Cape in hand → +100HP (flip the mirror race, mogja_j's move)")
                            return [i] if sel.minCount <= 1 else [i]
    ...
    # 14. v8 PROMOTE TRIGGER (MAIN) — if active is a weak basic (<=100hp, non-Dragapult) AND a benched
    #    Dragapult is ready (evolved + Fire+Psychic) → Switch-item or RETREAT to promote it.
    #    Planner gate: active.hp<=100 (don't displace a useful Latias/Fez wall). THE behavioral fix.
    if active is not None and active.id != DRAGAPULT and getattr(active, "hp", 0) <= 100:
        def _ready2(c):
            es = set(int(e) for e in (c.energies or []))
            return len(c.energies) >= 2 and 2 in es and 5 in es
        ready_drag = [c for c in (me.bench or []) if c is not None and c.id == DRAGAPULT and _ready2(c)]
        if ready_drag:
            si = play_idx(SWITCH_ITEM)
            if si is not None and sel.minCount <= 1:
                _log(turn, "Switch (→Dragapult)", f"active {card_name(active.id)} ({active.hp}hp) weak → Switch in ready Dragapult")
                return [si]
            rcost = card_retreat(active.id)
            if len(active.energies) >= rcost:
                for i, o in enumerate(opts):
                    if o.type == OptionType.RETREAT:
                        _log(turn, "Retreat (→Dragapult)", f"active {card_name(active.id)} ({active.hp}hp) weak → retreat, promote ready Dragapult")
                        return [i] if sel.minCount <= 1 else [i]
        ...
        if need_fire or need_psychic:
            ...
            for i, o in enumerate(opts):
                ...
                if card is None:
                    continue
                cid = card.id
                etype = getattr(B.card_table.get(cid), "energyType", None)
                # v12: Legacy Energy provides ANY type → fills the missing color AND grants -1 prize on KO.
                # Top priority when a color is missing (it's the root-cause fix for the 2-prize liability).
                if cid == LEGACY_ENERGY:
                    prio = 3
                elif etype == 5 and need_psychic:
                    prio = 2
            ...
            if best_i is not None and best_prio >= 1:
                ...
                _log(turn, "attach-color->Dragapult", f"active Dragapult needs {missing} for Phantom Dive → route (Legacy=any+prize-save) prio={best_prio}")
