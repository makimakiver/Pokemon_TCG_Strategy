"""Team Rocket's Honchkrow — research-driven heuristic pilot (v35 = v26 + DECK-TECH 26801 + Fezandipiti/Brave-Bangle closer).

V35 CHANGE vs v26 (the deck-tech pivot the pilot-only debate concluded was necessary):
  the pilot-only ceiling was ~73% because the closer-ready state (3-energy Porygon2 OR
  6-ammo+DCC Honchkrow) never co-occurs in play — Porygon2 is 90hp Fighting-weak (dies),
  and hoarding 6 supporters starves the mill engine. limitlesstcg 26801 breaks that zero-sum:
  * Fezandipiti ex (210hp, NO Fighting weakness) is a SURVIVING closer body. Its attack
    'Cruel Arrow' (CCC, 100 dmg to ANY opp Pokemon — snipe a benched basic for prize 6,
    NO gust needed) is a brand-new closer path that doesn't need hand supporters AND sits on
    a 210hp Fighting-safe wall. Its 'Flip the Script' ability draws 3 when we get KO'd ->
    directly fights the empty-hand-at-game-end problem.
  * Brave Bangle (tool): +30 dmg to a non-Rule-Box attacker's attacks -> Hammer In 100->130,
    RF +30/shot. A real damage boost on the Honchkrow closer.
  * Prime Catcher (item gust): the win-now gust for the closer mode (pull a soft basic for prize 6).
  DECK = 26801: 4 Murkrow/4 Honchkrow/2 Porygon/1 Porygon2/2 Articuno/1 Fezandipiti ex;
  4 each Proton/Ariana/Archer/Petrel/Giovanni/Transceiver/Roto-Stick, 3 Poke Pad, 1 Ultra Ball,
  1 Prime Catcher, 1 Lucky Helmet, 1 Brave Bangle, 3 Factory; 4 TR Energy + 4 Ignition.
  PILOT ADDS: develop Fezandipiti to CCC (3 energy) as a standing closer in racing mode;
  attach Brave Bangle to Honchkrow; allow Cruel Arrow as a prize-6 snipe; Prime Catcher = closer gust.
  (guard: abomasnow_cal)

Encodes the competitive piloting research

V26 CHANGE vs v25 (data-driven; v25 probe showed race-loss is now 51% of losses, ALL
at exactly 5 prizes taken — we race fine but can't close the 6th):
  vs Fighting in the endgame (someone at 4+ prizes taken = CLOSER MODE):
    C1  take ANY KO decisively (+300) — don't fumble the close by hoarding/developing;
        bank prizes toward 6 instead of stalling.
    C2  WIN-NOW GUST: Giovanni that pulls a soft benched target for the decisive prize
        jumps to top priority (9500) — close on an easy basic instead of chasing the
        340HP mega OHKO we may never amass 6 supporters for.
  Builds on v25's anti-donk. Inert in the opening and in non-Fighting matchups.
  (guard: abomasnow_cal)

Encodes the competitive piloting research

V25 CHANGE vs v14 (data-driven; tools/loss_probe.py, seat-bug-fixed, n=300):
  100% of our no-active losses vs Mega Lucario are GOING SECOND, and 36/59 never benched
  a second body (max board = 1). Only 8 basics in 60 -> ~35% of opens are exactly 1 basic;
  we can't bench during setup, and the Fighting deck OHKOs our lone active turn 3-5.
  So when the board is thin we MUST find + bench a body before anything else:
    D1  thin board (<=1 body) -> boost grabbing/benching ANY basic (TO_HAND, bench) so a
        safe body sits behind the active.
    D2  thin board + NO basic in hand + early (turn<=4) -> fire a search item / draw
        supporter NOW (Ultra Ball / Poke Pad / draw) to FIND a bench body. Anti-donk
        beats resource-preservation in the opening.
  Inert once the board is populated; does not touch the mill/Rocket-Feathers game.
  (guard: abomasnow_cal)
...
my_deck = ([463] * 4 + [891] * 3 + [473] * 2 + [474] * 1 + [414] * 2 +
           [140] * 1 +                       # Fezandipiti ex (210hp Darkness closer, no Fighting weakness)
           [1220] * 4 + [1216] * 4 + [1218] * 4 + [1219] * 4 + [1217] * 4 +
           [1152] * 4 + [1077] * 3 +         # Poke Pad (4), Roto-Stick (3) -- ammo search
           [1134] * 4 + [1097] * 3 +         # Transceiver, Night Stretcher (RECOVERY -> anti self-mill)
           [1121] * 1 + [1175] * 1 +         # Ultra Ball, Brave Bangle (+30 dmg on Honchkrow)
           [1257] * 3 +                       # Team Rocket's Factory
           [15] * 4 + [17] * 4)               # TR Energy, Ignition
# deck = HYBRID: v26's stable champion mill shell + the two closer-tech cards
# (Fezandipiti ex for Cruel Arrow snipe-close; Brave Bangle for Honchkrow damage).
# Cut vs champ: -1 Honchkrow (3), -1 Roto-Stick (3), -1 Miracle Headset, -1 Ultra Ball even
# -> +Fezandipiti ex +Brave Bangle. Keeps Night Stretcher (3) for recovery (26801's fatal omission).
...
FEZANDIPITI = _card_named("Fezandipiti ex")          # v35: 210hp Darkness closer (no Fighting weakness)
...
CRUEL_ARROW = _attack_named(FEZANDIPITI, "Cruel Arrow")    # v35: CCC, 100 dmg to ANY opp Pokemon (snipe closer)
...
def agent(obs_dict):
    ...
    # v25 ANTI-DONK flags (see docstring): going second we open with ~1 basic too often
    # and get donked before benching. When the board is thin, force a body onto the bench.
    going_second = (state.firstPlayer != my_index)
    have_basic_in_hand = any(c.id in BASIC_MONS for c in (my_state.hand or []))
    thin = n_bodies <= 1
    need_body = thin and not have_basic_in_hand and state.turn <= 4
    # v26 CLOSER MODE: late game vs Fighting (someone at 4+ prizes taken) we must CLOSE —
    # the 5-prize race-loss is now 51% of losses. Fire KOs decisively and gust-win-now.
    # prizes_we_took = 6 - op_prize_left ; prizes_opp_took = 6 - len(my_state.prize).
    closer = opp_fighting and (op_prize_left <= 2 or len(my_state.prize) <= 2)
    ...
    if context == SelectContext.MAIN:
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                is_cruel = (mine.id == FEZANDIPITI and aid == CRUEL_ARROW)   # v35: 100 to ANY opp Pokemon
                if is_rf and tr_ammo <= 0:
                    continue
                if is_rc and rc_fuel <= 0:
                    continue
                # v35: Cruel Arrow snipes ANY opp Pokemon (not just the active). Find the
                # juiciest target across active+bench; if any KO wins or sets up the close, take it.
                if is_cruel:
                    best_t = -1
                    best_t_hp = 999
                    for j, ot in enumerate(op_cards):
                        if ot is None:
                            continue
                        # 100 dmg (Brave-Bangle-independent; Fezandipiti is ex = Rule Box so
                        # Bangle doesn't boost it). KO anything <=100hp.
                        if ot.hp <= 100:
                            # prefer the one that wins or is the biggest prize
                            pcount = prize_count(ot)
                            if op_prize_left <= pcount:
                                best_t = j; best_t_hp = -1; break
                            if pcount >= 2 or best_t < 0:
                                best_t = j
                    if best_t < 0:
                        continue          # no snipeable target -> skip this attack line

                opp = op_cards[0]
                if opp is None:
                    continue
                # v35: Cruel Arrow can target a benched basic instead of the active. If we
                # found a snipe target above (best_t), evaluate against THAT one.
                if is_cruel and best_t >= 0:
                    opp = op_cards[best_t]
                ...
                if is_rf:
                    ...
                    dmg = 100              # Cruel Arrow: flat 100 to the chosen target (no weakness/res for bench)
                else:
                ...
                kos = opp.hp <= dmg
                # v35: a Cruel Arrow snipe from the SURVIVING 210hp Fezandipiti closer is exactly
                # the prize-6 line the pilot-only deck couldn't reach. Boost it heavily.
                if is_cruel:
                    score += 350
                    if kos and op_prize_left <= prize_count(opp):
                        score = 60000      # game-winning snipe
                if kos:
                    score += 500
                ...
                if is_rf:
                    score -= 4 * tr_ammo
                # v26 CLOSER: in the endgame race, take ANY prize decisively — don't fumble
                # the close. (A winning KO is already 50000; this edges a non-winning KO above
                # hoarding/developing so we bank prizes toward 6.)
                if closer and kos:
                    score += 300
                ...
                if score > best:
                    best = score
                    plan.attacker = i
                    plan.target = best_t if is_cruel and best_t >= 0 else 0
                    plan.attack_id = aid
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if pokemon.id == HONCHKROW:
            ...
            # v35: Fezandipiti ex is the SURVIVING closer (210hp, no Fighting weakness).
            # In racing/closer mode, develop it to CCC (3 energy) so Cruel Arrow is online
            # as a standing prize-6 snipe. Otherwise treat it as a modest wall.
            score += 320
            racing = opp_fighting and (6 - op_prize_left) >= 3
            if racing and len(pokemon.energies) < 3:
                score += 260
        ...
        if is_active:
            score += 40
        need = 2 if pokemon.id == HONCHKROW else 3 if pokemon.id == PORYGON2 else 2
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if card.id == MURKROW:
                        ...
                        # v35: Fezandipiti ex is a 210hp Fighting-safe wall + the closer body.
                        # Bench it preferentially vs Fighting so the closer line is in reserve.
                        score = 135 if opp_fighting else 70
                        ...
                        score = 50
                    if thin and card.id in BASIC_MONS:
                        score += 500     # v25 anti-donk: bench any body to survive the opening
                    ...
                    if cid in ATTACKERS:
                        ...
                        if thin:
                            score += 500     # v25 anti-donk: grab a bench body first
                ...
                if ct == CardType.POKEMON:
                    ...
                    if cid == GIOVANNI:
                        ...
                        if active_lethal:
                            ...
                            # v26 CLOSER: win-now gust — pull a soft benched target for the
                            # decisive prize. Top priority in the endgame race.
                            if closer and gusts:
                                score = 9500
                    elif cid == PROTON:
                    ...
                    # v25 anti-donk: thin board + no basic in hand -> dig NOW (any draw/tutor)
                    if need_body:
                        score = max(score, 12000)
                    ...
                    if plan.attack_id not in (-1, None) and plan.remain_hp <= 0:
                        score = min(score, 300)
                    # v25 anti-donk: thin board + no basic in hand -> fire a search item NOW
                    # (Ultra Ball / Poke Pad find a Pokemon) to get a bench body down.
                    if need_body:
                        score = 20000
            ...
            if card is not None and card.id in TOOLS:
                ...
                # v35: Brave Bangle (+30 dmg to non-Rule-Box attacks) -> prioritize Honchkrow,
                # our Fighting-safe damage body (Hammer In 100->130, RF boosted). Never on
                # Fezandipiti (it's ex = Rule Box, Bangle does nothing there).
                if card.id == 1175:   # Brave Bangle
                    if pokemon is not None and pokemon.id == HONCHKROW:
                        score += 1500
                    elif pokemon is not None and pokemon.id == FEZANDIPITI:
                        score = 50   # wasted on the ex; skip
            else:
