"""Team Rocket's Honchkrow — research-driven heuristic pilot (v27 = v26 + R-COMMAND CLOSER).

V27 CHANGE vs v26 (data-driven; v26 race-loss diagnostic: in 48/48 five-prize losses
our active is EMPTY and our hand is EMPTY (0 cards). We burn hand supporters as RF
ammo, take 5 prizes, then can't find the 6th KO. The deck's intended HAND-INDEPENDENT
closer is R Command (Porygon2, 20 x #TR-supporters in DISCARD) — it runs off the discard
pile, not the hand, so it's exactly the finisher for the empty-hand endgame. But we never
develop Porygon2 to deliver it.):
    RC1  in CLOSER MODE (>=4 prizes taken) with rc_fuel >= 8, prefer developing the
         Porygon2 closer: boost evolving Porygon->Porygon2 (7000) and feeding it the 3rd
         energy so it can fire R Command next turn.
    RC2  in CLOSER MODE, an R Command attack line that KOs gets a big bonus (+400) since
         it is the hand-free finisher.
    Guardrail: only develop Porygon2 as a CLOSER (rc_fuel>=8), never as a passive body, so
    we don't re-introduce v23's free-prize problem.
  (guard: abomasnow_cal)
...
def agent(obs_dict):
            ...
            for aid in (cd.attacks or []):
                ...
                if kos:
                    if is_rc:
                        ...
                        # v27: in closer mode the R Command finisher is worth a lot — it runs
                        # off the discard pile so it works in the empty-hand endgame.
                        if closer:
                            score += 400
                    elif is_hammer:
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if len(pokemon.energies) >= need:
            score -= 220
        # v27 R-COMMAND CLOSER: in closer mode with discard fuel, push energy onto a
        # Porygon2 (needs 3 colorless for R Command) so it can deliver the hand-free finisher.
        if closer and pokemon.id == PORYGON2 and rc_fuel >= 8 and len(pokemon.energies) < 3:
            score += 500
        return score
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # v27 R-COMMAND CLOSER: in closer mode with enough discard fuel, develop Porygon2
            # so it can deliver the hand-independent R Command finisher. Guardrail: only as a
            # closer (rc_fuel>=8) — never develop Porygon2 as a passive body vs Fighting.
            if closer and pokemon is not None and pokemon.id == PORYGON and rc_fuel >= 8:
                score = 7000 + (len(pokemon.energies) if isinstance(pokemon, Pokemon) else 0) * 20
