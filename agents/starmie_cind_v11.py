...
def agent(obs_dict):
    ...
    # v11 NARROW FORCE-BENCH — prevent the turn-2 no-active board-wipe (traced: 12/37 Lucario losses die T2 with empty bench).
    # Fires ONLY when bench is empty (wipe-risk state) early -> force Poffin to bench 2 basics. Narrow gate => no tempo cost
    # in normal games (bench isn't empty). Unlike v3 (broad Poffin gate) which regressed.
    bench_count = sum(1 for c in (me.bench or []) if c is not None)
    if turn <= 2 and bench_count == 0 and POFFIN in hand:
        for i, o in enumerate(opts):
            if o.type == OptionType.PLAY:
                c = get_card(obs, AreaType.HAND, o.index, st.yourIndex)
                if c is not None and c.id == POFFIN:
                    _log(turn, "Poffin (empty-bench save)", "bench empty T<=2 -> bench 2 basics, prevent no-active wipe")
