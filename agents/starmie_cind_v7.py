...
def agent(obs_dict):
    ...
    # v7 CAPE-TO-STARMIE-EX — attach the single Hero's Cape to the ACTIVE Mega Starmie ex (330->430hp).
    # Replaces v4's active-basic Cape (wasted: 70->170hp Staryu still dies to 270 Mega Brave, Cape discarded
    # with it). 430hp Starmie ex SURVIVES Lucario Brave(270)+Jab(130)=400 -> wins the prize race; also tanks
    # mirror 2xJetting(240), Dragapult 2xPhantomDive(400). Universally optimal Cape target. No opp-gate.
                ...
                if o.type == OptionType.ATTACH:
                    ...
                    if card is not None and card.id == HEROS_CAPE and getattr(o, "inPlayArea", None) == 4:
                        ...
                        if tgt is not None and tgt.id == STARMIE_EX:
                            _log(turn, "attach Hero's Cape (->Starmie ex)", "active Starmie ex + Cape -> 430hp (win the Lucario/mirror/Dragapult race)")
