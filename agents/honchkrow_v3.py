"""Tuned agent for Rocket's Honchkrow.

The deck is a combo, not a beatdown. Its real win condition is invisible to a
damage-stat pilot:

  Team Rocket's Honchkrow -- 'Rocket Feathers' [CC]: does 60 damage for EACH
  "Team Rocket" Supporter you discard from your hand (listed damage = 0).

So the plan this agent encodes:
  1. Evolve Murkrow -> Honchkrow and power it with a single special energy
     (Team Rocket's Energy gives 2; Ignition on an Evolution gives 3).
  2. HOARD Team Rocket Supporters in hand as ammunition (don't burn them for
     their text) -- fetched by Murkrow 'Deceit', Transceiver, Petrel, Roto-Stick,
     and refilled by Ariana's draw-to-8.
  3. Fire Rocket Feathers, discarding enough ammo to KO (60 x N).
Articuno's 'Repelling Veil' walls the bench; Hammer In [DCC]=100 is the vanilla
fallback when ammo is short.

Built on the generic bare_agent engine (runtime stats, evolve-ASAP, feed-the-
attacker), with Honchkrow-specific attack valuation, supporter hoarding, and
ammo-discard handling layered on top.
"""
import json
import os
...
RF_ATTACK = _attack_named(HONCHKROW, "Rocket Feathers")    # 60 x TR-Supporters discarded
...
# v2: Porygon2 'R Command' = 20 x (Team Rocket Supporters in DISCARD pile). Listed damage
# is 0 (scaling, like Rocket Feathers), so the generic damage-stat loop skips it; we
# special-case it as an offensive finisher that converts spent ammo into damage.
PORYGON2 = _card_named("Team Rocket's Porygon2")
R_COMMAND = _attack_named(PORYGON2, "R Command")
RC_PER_CARD = 20

# v2: Rocket Feathers ammo discard order (research Section D): discard lowest-value
# supporters first, keep the broadest tutors. Proton -> Archer -> Ariana -> Giovanni -> Petrel.
_DISCARD_RANK = {}
for _r, _nm in enumerate(["Team Rocket's Proton", "Team Rocket's Archer",
                          "Team Rocket's Ariana", "Team Rocket's Giovanni",
                          "Team Rocket's Petrel"]):
    _cid = _card_named(_nm)
    if _cid is not None:
        _DISCARD_RANK[_cid] = _r
...
# Honchkrow's attacker is a Stage-1 with a listed damage of 0, so the generic
# detector misses it — make sure it counts as an attacker.
if HONCHKROW:
    ATTACKERS.add(HONCHKROW)
# v2: Porygon2's R Command also lists 0 damage — make it count as an attacker too.
if PORYGON2:
...
def rocket_feathers_damage(attacker, target, ammo):
    """Honchkrow 'Rocket Feathers': 60 x (TR Supporters discarded), with weakness."""
    return _apply_weakness(attacker, RF_PER_CARD * max(0, ammo), target)


def r_command_damage(attacker, target, tr_in_discard):
    """Porygon2 'R Command': 20 x (TR Supporters in discard pile), with weakness."""
    return _apply_weakness(attacker, RC_PER_CARD * max(0, tr_in_discard), target)
...
def agent(obs_dict):
    ...
    # Team Rocket Supporters in hand = Rocket Feathers ammunition (60 dmg each).
    tr_ammo = sum(hand_counts[c] for c in TR_SUPPORTERS)
    # v2: Team Rocket Supporters in the discard pile = R Command damage (20 dmg each).
    tr_in_discard = sum(1 for c in (my_state.discard or [])
                        if c is not None and c.id in TR_SUPPORTERS)
    ...
    if context == SelectContext.DISCARD and select.minCount == 0 and select.option:
        ...
        if tr_idx and len(tr_idx) == sum(1 for o in opts if o.type == OptionType.CARD):
            ...
            # v2: discard lowest-value supporters first, keep the broadest tutors.
            def _disc_rank(k):
                o = opts[k]
                c = get_card(obs, o.area, o.index, o.playerIndex)
                return _DISCARD_RANK.get(c.id if c else -1, 99)
            tr_idx.sort(key=_disc_rank)
            return tr_idx[:need]
            ...
            for aid in (cd.attacks or []):
                ...
                if not can_pay(attached_types, cost):
                    ...
                    if not state.energyAttached:
                        ...
                        continue
                # Rocket Feathers lists 0 damage but deals 60 per TR Supporter discarded.
                ...
                for j, opp in enumerate(op_cards):
                    ...
                    if is_rf:
                        ...
                        dmg = r_command_damage(mine, opp, tr_in_discard)
                    else:
                    ...
                    # over-discard supporters for an already-lethal Rocket Feathers.
                    if is_rf:
                        score -= 6 * tr_ammo
                    # v2: R Command spends NO hand resources (uses the discard pile), so when
                    # both attacks KO, prefer it to preserve Rocket Feathers ammo (research rule 5).
                    if is_rc:
                        score += 40
                    # v3: when Hammer In (100, no discard) also KOs, prefer it over Rocket Feathers
                    # to bank Team Rocket Supporters for future RF/R Command (research rule 5).
                    if is_hammer and opp.hp <= dmg:
                        score += 30
                    if i == 0:
                        score += 200
    ...
    def energy_score(pokemon, is_active):
        ...
        if pokemon is None:
            return -1
        # Energy is scarce (all special), so concentrate it on the Honchkrow line and
        # don't strand it on Articuno/Porygon (walls that can't pay our attacks).
        if pokemon.id == HONCHKROW:
            ...
            score += 20           # Articuno/Porygon: don't waste special energy here
        if is_active:
            score += 40
        # Rocket Feathers only costs [CC]; once an attacker has 2, spread to a backup.
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if cid in ATTACKERS:
                        ...
                        score = 200          # fetch ammo for Rocket Feathers
                    ...
                    if cid in ATTACKERS or cid in BASIC_MONS:
                        ...
                        score = 60           # Rocket Feathers fuel: discard ammo first
                    elif cid in ALL_ENERGY:
