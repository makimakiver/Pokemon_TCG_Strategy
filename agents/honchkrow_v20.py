...
TORMENT = _attack_named(MURKROW, "Torment")                # DC, 30 + lock 1 of defender's attacks
_ABOM_LINE = {723, 722, 418, 419}                          # Mega Abomasnow ex + Snover/Abomasnow
RF_PER_CARD = 60
RC_PER_CARD = 20
...
def agent(obs_dict):
    ...
    # v20 CASE-SPECIFIC (snowman): the opponent is an Abomasnow / Water box. Detected by
    # the Abomasnow/Snover line or a Water-majority board. We turn on Torment-lock + energy
    # dig ONLY here so the common Lucario matchup is untouched.
    _board = [c for c in ([op_active] + list(op_state.bench)) if c is not None]
    opp_snowman = any(c.id in _ABOM_LINE for c in _board) or (
        len(_board) >= 2 and sum(1 for c in _board
                                 if card_table.get(c.id) and card_table[c.id].energyType == EnergyType.WATER)
        >= max(1, len(_board) // 2))
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if cid == GIOVANNI:
                        ...
                        if deck_count <= 8:
                            ...
                            score = 1200
                        # v20: vs the snowman we die to the race, not deck-out — when our
                        # attacker is starved, dig HARD for the (untutorable) energy. Gated to
                        # snowman so the common Lucario matchup keeps its deck-out discipline.
                        if opp_snowman and active_starved and deck_count >= 10:
                            score = 6000
                    elif cid == ARCHER:
            ...
            # Torment's follow-up: choose WHICH of the opponent's attacks to lock — pick the
            # highest-listed-damage one (Frost Barrier 200 > Hammer-lanche 0).
            if context == SelectContext.DISABLE_ATTACK:
                a = attack_table.get(o.attackId)
                scores.append((getattr(a, "damage", 0) or 0) + 1)
                continue
            score = 1000
            ...
            if o.attackId == DECEIT and plan.attack_id in (-1, DECEIT):
                score = 900
            # v20 TORMENT-LOCK vs snowman: if we can't OHKO the wall this turn, lock the
            # opponent's main attack (Frost Barrier). The wall's only other attack
            # (Hammer-lanche) lists 0 damage, so a value-reading pilot won't use it -> it
            # whiffs the turn, we don't get OHKO'd, and we set up the kill.
            if (o.attackId == TORMENT and opp_snowman
                    and not (plan.attack_id not in (-1, None) and plan.remain_hp <= 0)):
                score = 1700
