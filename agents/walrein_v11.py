...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
        ...
        # v11 DAWN FORCE-PLAY: Dawn (1231) tutors Basic+Stage1+Stage2 -> assembles the Walrein line.
        # bare under-uses supporters (like Rare Candy). Force it when the line is incomplete.
            ...
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    ...
                    if c is not None and c.id == 1231:
                        return [i] if select.minCount <= 1 else [i]
        # v11 CRUSHING-HAMMER denial (any energy; skip Enhanced 1081 which hurt dragapult).
        _oa = state.players[1-my_index].active[0] if state.players[1-my_index].active else None
        if _oa is not None and hasattr(_oa,"energies") and len(_oa.energies)>0 and 1120 in hand_ids:
            opts = select.option or []
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    c = get_card(obs, AreaType.HAND, o.index, my_index)
