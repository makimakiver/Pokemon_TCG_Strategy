"""Team Rocket's Honchkrow — research-driven heuristic pilot (v29 = v26 + ARTICUNO WALL).

V29 CHANGE vs v26 (data-driven; targets the board-collapse bucket, 22% of losses. Honchkrow
is Fighting-RESISTANT, and so is Articuno (120hp, weak=Lightning, res=Fighting -30).
Articuno can't attack (no Water energy) so it's a pure wall — but in a mill deck, eating one
attack WITHOUT giving up a prize is a free turn, and that's exactly the tempo that prevents
the early board-collapse sweeps when going second):
    W1  vs Fighting, BENCH Articuno preferentially (it's a 120hp tank) so a wall is in
         reserve.
    W2  vs Fighting, when our active is about to die (HP low / no good attack) and Articuno
         is benched, RETREAT/PROMOTE to Articuno to wall the next hit instead of feeding a
         free prize+tempo. The wall banks a deck-out turn.
  Guardrail: Articuno never attacks (no Water), so this is pure defense; once Honchkrow is
  ready, switch back. Inert in non-Fighting matchups. (guard: abomasnow_cal)
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    if o.playerIndex == my_index:
                        ...
                        # v29 WALL: vs Fighting, if our active is about to be KO'd (no lethal
                        # attack this turn) and Articuno is benched, promote the 120hp Fighting-
                        # resistant Articuno to wall the next hit and bank a deck-out turn.
                        active_lethal = (plan.attack_id not in (-1, None) and plan.remain_hp <= 0)
                        if (opp_fighting and not active_lethal and card.id == ARTICUNO
                                and my_state.active and my_state.active[0]
                                and my_state.active[0].hp <= 60):
                            score += 400
                    else:
                    ...
                    if card.id == MURKROW:
                        ...
                        score = 30
                        # v29 WALL: vs Fighting, bench the 120hp Fighting-resistant Articuno
                        # so a wall is in reserve to absorb a hit and prevent board-collapse.
                        if opp_fighting:
                            score = 100
                    elif card.id in BASIC_MONS:
                        score = 80
