...
CONKELDURR = 115            # v3b: Seek Inspiration copy target (250)
CIPHERMANIAC = 1188         # v3b: top-card setup
...
def agent(obs_dict):
    ...
    churning = turn_actions > 35    # v3b anti-loop: past 35 actions/turn, stop digging (force END)
    deck_count_var = getattr(my_state, 'deckCount', 99)
    slowking_in_play = any(c is not None and c.id == SLOWKING for c in (list(my_state.active)+list(my_state.bench)))
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                for j, opp in enumerate(op_cards):
                    ...
                    # v3b COPY ENGINE: Seek (dmg=0) copies top card; if Conkeldurr still in deck,
                    # score Seek as 250 so the pilot fires it (v2 never did). Engine resolves the copy.
                    if is_seek and dmg <= 0 and field_counts.get(CONKELDURR, 0) == 0 and deck_count_var > 4:
                        dmg = 250
                    if dmg <= 0:
                        continue
                    ...
                    if mine.id == SLOWKING:               # v2 lever 2: prefer the 1-prize identity attacker
                        score += 120
                    if is_seek and dmg >= 200:              # v3b: a 250-copy Seek is the deck's best line
                        score += 600
                    if score > best:
                        best = score
                        plan.attacker = i
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # v3b anti-loop: once churning, STOP playing cards (we're cycling search/tutors).
            if card is not None and churning:
                score = -5000
                ...
                if ct == CardType.POKEMON:
                    ...
                    score = 3000
                    if card.id == CIPHERMANIAC and slowking_in_play and field_counts.get(CONKELDURR, 0) == 0 and deck_count_var > 4:
                        score = 9000    # v3b: sets Conkeldurr on top for the 250 copy
            ...
            score = 200000 if (stalling or churning) else -1000
