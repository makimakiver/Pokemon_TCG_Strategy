...
# ==KNOBS_START==
KNOBS = {
    'item_score_deck_le_12': 200,
    'item_score_deck_le_20': 1500,
    'item_score_deck_default': 3500,
    'item_lethal_cap': 300,
    'energy_grab_when_starved': 450,
    'giovanni_lethal_score': 50,
    'ariana_gate_low': 8,
    'ariana_gate_mid': 16,
    'ariana_score_low': 150,
    'ariana_score_mid': 1200,
}
# ==KNOBS_END==
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if cid in ATTACKERS:
                        ...
                        score = KNOBS["energy_grab_when_starved"] if active_starved else (120 if not state.energyAttached else 60)
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    score = 20000
                    if cid in BASIC_MONS and n_bodies >= 3:
                        score = 30
                    ...
                    if cid == GIOVANNI:
                        ...
                        if active_lethal:
                            score = KNOBS["giovanni_lethal_score"]
                        else:
                        ...
                        if deck_count <= KNOBS["ariana_gate_low"]:
                            score = KNOBS["ariana_score_low"]
                        elif deck_count <= KNOBS["ariana_gate_mid"] and len(my_state.hand or []) > 2:
                            score = KNOBS["ariana_score_mid"]
                    elif cid == ARCHER:
                    ...
                    if deck_count <= 12:
                        score = KNOBS["item_score_deck_le_12"]
                    elif deck_count <= 20:
                        score = KNOBS["item_score_deck_le_20"]
                    else:
                        score = KNOBS["item_score_deck_default"]
                    if plan.attack_id not in (-1, None) and plan.remain_hp <= 0:
                        score = min(score, KNOBS["item_lethal_cap"])
