"""Mega Lucario ex pilot (top-100 meta deck) on the generic bare-setup engine.

Ships the Mega Lucario ex 60 (most common list among top-team winners) and reuses
the same archetype-agnostic engine as bare_agent.py (runtime stats, evolve-ASAP,
feed-the-attacker energy). Originally: a generic "bare setup" agent piloting ANY deck.
...
# ---- Mega Lucario ex deck (the top-100 meta deck) ----
# The most-common list among the top-100 teams' Honchkrow-free winning replays
# (23x; extracted to out/mega_lucario_deck.json). Riolu -> Mega Lucario ex Fighting
# line + Makuhita/Hariyama, Solrock/Lunatone support, Gravity Mountain stadium,
# Hero's Cape ACE SPEC. Overridable via BARE_DECK so this engine can pilot any deck.
...
if _deck_path and os.path.exists(_deck_path):
    ...
    my_deck = (
        [6]*13 +                      # Basic {F} Energy
        [677]*3 + [678]*4 +           # Riolu -> Mega Lucario ex
        [673]*2 + [674]*2 +           # Makuhita -> Hariyama
        [675]*2 + [676]*3 +           # Lunatone / Solrock (support)
        [1102]*4 + [1152]*4 +         # Dusk Ball / Poke Pad (search)
        [1192]*4 + [1227]*4 +         # Carmine / Lillie's Determination (draw)
        [1141]*4 + [1142]*4 +         # Premium Power Pro / Fighting Gong
        [1123]*2 + [1182]*2 +         # Switch / Boss's Orders (gust)
        [1252]*2 + [1159]             # Gravity Mountain / Hero's Cape (ACE SPEC)
    )
