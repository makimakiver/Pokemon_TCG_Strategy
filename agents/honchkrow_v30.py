"""Team Rocket's Honchkrow — research-driven heuristic pilot (v30 = v26 + HARD ANTI-SELF-MILL).

V30 CHANGE vs v26 (targets the self-deck-out LOSS bucket, ~7% of losses = ~21/300. The deck's
win condition is making the OPPONENT deck out, but our own draw/search engine sometimes
mills us faster than them, so we deck out first. v14's throttle (H1/H4) starts at deck<=20;
v30 tightens it HARD: once the deck is low, stop ALL self-drawing unless we are donk-risky
or need to find a closer-KO, so we outlast the opponent and flip deck-out losses to wins):
    M1  raise the no-draw floor: deck<=14 -> do NOT play items/draw-supporters (score ~50)
         unless the play finds a body (anti-donk) or lines up a lethal this turn.
  (guard: abomasnow_cal)
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if deck_count <= 12:
                        ...
                        score = 3500
                    # v30 HARD ANTI-SELF-MILL: once the deck is low, STOP self-drawing unless we
                    # are donk-risky (need_body) or have a lethal lined up. Flips deck-out losses.
                    if deck_count <= 14 and not need_body:
                        lethal = (plan.attack_id not in (-1, None) and plan.remain_hp <= 0)
                        score = 3500 if lethal else 50
