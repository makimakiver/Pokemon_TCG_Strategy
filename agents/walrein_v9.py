...
os.environ.setdefault("BARE_DECK", os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "decks", "deck_walrein_v2.json"))
...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
        ...
        # v9 CRUSHING HAMMER FORCE-PLAY: keep opp <=2 energy so the Frigid Fangs lock holds
        # (the root cause of weak matchups: opp ramps to 3+ energy -> lock fails). bare under-plays
        # it like Rare Candy. Fire it whenever opp active has energy and we hold one.
        op_active_id = (state.players[1-my_index].active[0].id if (state.players[1-my_index].active and state.players[1-my_index].active[0]) else None)
        opp_has_energy = False
        _oa = state.players[1-my_index].active[0] if state.players[1-my_index].active else None
        if _oa is not None and hasattr(_oa,"energies") and len(_oa.energies)>0:
            opp_has_energy = True
        if opp_has_energy and 1120 in hand_ids:
            opts = select.option or []
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    c = get_card(obs, AreaType.HAND, o.index, my_index)
                    if c is not None and c.id == 1120:
                        return [i] if select.minCount <= 1 else [i]
