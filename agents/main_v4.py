...
# ---- Buddy Blast overlay (the ONE fix from replay analysis) ----
# Typhlosion's Buddy Blast deals 40 + 60 per Ethan's Adventure (id 1215) in the
# discard, but only costs a single {R} energy.  The generic engine reads the
# base 40 from the card table and never picks it.  These helpers compute the
# real fuel-aware damage.
_HAS_TYPHLOSION = 354 in _deck_set
_TYPHLOSION_ID = 354
_ETHANS_ADVENTURE = 1215
_BUDDY_BLAST_AID = None
if _HAS_TYPHLOSION:
    _typh = card_table.get(_TYPHLOSION_ID)
    if _typh and _typh.attacks:
        for _aid in _typh.attacks:
            _a = attack_table.get(_aid)
            if _a and (getattr(_a, "damage", 0) or 0) == 40:
                _BUDDY_BLAST_AID = _aid
                break


def _ethan_in_discard(my_state):
    return sum(1 for c in (my_state.discard or []) if c.id == _ETHANS_ADVENTURE)
...
def attack_damage(attacker, atk, target, ethan_fuel=0):
    # Buddy Blast: 40 + 60 per Ethan's Adventure in discard (Typhlosion only).
    if (_HAS_TYPHLOSION and attacker.id == _TYPHLOSION_ID
...
def agent(obs_dict):
    ...
    ethan_fuel = _ethan_in_discard(my_state) if _HAS_TYPHLOSION else 0
