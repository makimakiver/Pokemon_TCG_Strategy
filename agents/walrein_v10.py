...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
        ...
        # v10 DENIAL FORCE-PLAY: keep opp <=2 energy so the Frigid Fangs lock holds. Force BOTH
        # Enhanced Hammer (1081, special energy) and Crushing Hammer (1120, any energy, coin flip)
        # whenever the opp active has energy. bare under-plays them (like Rare Candy).
        _oa = state.players[1-my_index].active[0] if state.players[1-my_index].active else None
        opp_has_energy = (_oa is not None and hasattr(_oa,"energies") and len(_oa.energies)>0)
        if opp_has_energy:
            opts = select.option or []
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    c = get_card(obs, AreaType.HAND, o.index, my_index)
                    if c is not None and c.id in (1081, 1120):
                        return [i] if select.minCount <= 1 else [i]
