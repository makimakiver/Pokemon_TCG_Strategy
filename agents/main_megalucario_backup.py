...
# Deck inlined for single-file submission (was loaded from deck.csv).
my_deck = [
    673, 673, 674, 674, 675, 675, 676, 676, 676, 677, 677, 677, 678, 678, 678, 678, 1102, 1102, 1102, 1102, 1123, 1123, 1141, 1141, 1141, 1141, 1142, 1142, 1142, 1142, 1152, 1152, 1152, 1152, 1159, 1182, 1182, 1192, 1192, 1192, 1192, 1227, 1227, 1227, 1227, 1252, 1252, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6,
]
...
def pokemon_score(pokemon: Pokemon) -> int:
    ...
    if id == 112 and len(pokemon.energies) >= 1:  # Munkidori
        score += 300
    # EXP5: meta-aware target priority (IDs verified in EN_Card_Data.csv).
    if id == 121:            # Dragapult ex (Tier 1 attacker, Stage 2)
        score += 400
    elif id == 272 or id == 29:  # Lillie's Clefairy ex / Sinistcha ex (Bakegakure)
        score += 350
    score += pokemon.hp
    return score
                ...
                for a in range(2):
                    ...
                    if my_pokemon.id == Mega_Lucario_ex:
                        if a == 0:
                            ...
                            base_damage = 130
                            base_score += 60 * min(
                                3, discard_counts[Basic_Fighting_Energy]
                            )
                        else:
                            energy_required = 2
                            base_damage = 270
        ...
        if o.type == OptionType.NUMBER:
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Switch:
                    ...
                    if plan.target >= 1:
                        score = 4200 if plan.remain_hp <= 0 else 2600
                    else:
                        score = -1
                    ...
                    op_has_stage2 = any(
                        ...
                        for p in [op_state.active[0]] + list(op_state.bench)
                    )
                    if stadium_id == 0 and not op_has_stage2:
                        score = -1
