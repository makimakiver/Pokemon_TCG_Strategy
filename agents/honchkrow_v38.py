...
my_deck = ([463] * 4 + [891] * 3 + [473] * 2 + [474] * 1 + [414] * 2 +
           ...
           [1152] * 4 + [1077] * 4 + [1134] * 4 + [1097] * 3 +
           [1121] * 1 + [1122] * 2 +          # Ultra Ball, +2 Pokegear 3.0 (density injection)
           [1257] * 3 + [15] * 4 + [17] * 3)  # Factory, TR Energy, -1 Ignition Energy (trim to 7)
assert len(my_deck) == 60, f"deck has {len(my_deck)} cards"
# v38: v26 deck + planner's H1 density injection (-1 Miracle Headset, -1 Ignition Energy,
# +2 Pokegear 3.0). Net-neutral Pokemon count (unchanged body composition). Pokegear tutors a
# Supporter (top 7) -> consistency for the slow matchups without starving Lucario's bodies.
...
POKEGEAR = _card_named("Pokégear 3.0")          # v38: top-7 Supporter tutor (density injection)
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if deck_count <= 12:
                        ...
                        score = 3500
                    # v38 DENSITY POSTURE (vs non-Fighting = the slow/mill matchups): we WANT
                    # consistency over deck-preservation here, so raise the dig floor and play
                    # items aggressively mid-game. vs Fighting keep the original conservative taper.
                    if not opp_fighting and deck_count > 12:
                        score = max(score, 3200)
                    # v38 Pokegear priority: vs non-Fighting, if no draw Supporter is in hand,
                    # fire Pokegear to tutor one (it finds the engine piece).
                    if not opp_fighting and cid == POKEGEAR:
                        have_draw_supporter = any(hand_counts.get(s, 0) > 0 for s in (ARIANA, PETREL, GIOVANNI, PROTON))
                        if not have_draw_supporter:
