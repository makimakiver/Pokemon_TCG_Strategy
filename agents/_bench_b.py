# AUTO-GENERATED for pokemon_bench.py from meta_opp.py — self-contained generic pilot,
# reads BENCH_B_DECK so side A and side B never share deck state (collision-proof). DO NOT add bare_agent.
...
The 60-card deck is loaded from the META_DECK env var (path to a JSON list of ids),
...
# ---- Load the deck this bare agent should pilot ----
_deck_path = os.environ.get("BENCH_B_DECK", "")
if _deck_path and os.path.exists(_deck_path):
    my_deck = json.load(open(_deck_path))
else:  # fallback: the Palace deck, so the module is always importable
    my_deck = [1]*19 + [11]*4 + [14]*4 + [18]*4 + [344]*4 + [345]*4 + \
