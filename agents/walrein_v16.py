...
def agent(obs_dict):
    ...
    if context == 0:  # MAIN
            ...
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    ...
                    if c is not None and c.id == 1231:
                        return [i] if select.minCount <= 1 else [i]
        # v16 PER-MATCHUP Fangs/Megaton (planner spec Round 5): v15's single generic policy hurt
        # bolt (-6) and was strictly harmful on hydrapple. Per-matchup gates fix both.
        DRAGAPULT_IDS = {120, 121}            # Drakloak, Dragapult ex
        BOLT_IDS = {63}                        # Raging Bolt ex (dragon ramp)
        HYDRAPPLE_IDS = {918, 710, 919, 347, 150}  # Bayleef/Meganium line + Hydrapple ex
        ...
        is_dragapult = any(i in DRAGAPULT_IDS for i in op_all_ids)
        is_bolt = any(i in BOLT_IDS for i in op_all_ids)
        is_hydrapple = any(i in HYDRAPPLE_IDS for i in op_all_ids)
        gated = is_dragapult or is_bolt or is_hydrapple
        ...
        if gated and active_is_walrein:
            ...
            if atk_idx:
                ...
                # v16 PER-MATCHUP RULE
                want = None
                if is_hydrapple:
                    # 2x Grass OHKO means Fangs never saves Walrein -> Megaton ALWAYS (race).
                    want = "Megaton Fall"
                elif is_bolt:
                    # Bolt ramps FAST: Fangs ONLY at energy 1-2 (buy one turn); Megaton at 3+.
                    if op_active_energy <= 2:
                        want = "Frigid Fangs"
                    else:
                        want = "Megaton Fall"
                elif is_dragapult:
                    # v15 closed-loop policy unchanged: Fangs to prevent KO, else Megaton to race.
                    if opp_can_ohko and op_active_energy <= 2:
                        ...
                        want = "Frigid Fangs"
                # override: if the chosen action picked an ATTACK, swap it to `want` (if available)
                if want in atk_idx:
                    want_i = atk_idx[want]
                    # did bare choose an ATTACK?
                    bare_atk = None
                    for idx in chosen:
