...
ITCHY_POLLEN = 323              # Budew 'Itchy Pollen' (10dmg) — a wall attack, NOT an attacker
...
def agent(obs_dict):
    ...
    line_developing = DREEPY in in_play or DRAKLOAK in in_play or drag_in_play
    active_is_drag = active is not None and active.id == DRAGAPULT
    active_is_budew = active is not None and active.id == BUDEW
    ...
    # 0. v10 BUDEW-ATTACK SUPPRESSION (the behavioral root-cause fix). The replay diff shows our losses
    #    waste T4-T10 attacking with Budew's ItchyPollen (10dmg) while the Dragapult line develops on the
    #    bench — delaying first Dragapult attack to T9-16 (mogja_j evolves+attacks T3). Budew is a setup
    #    SACRIFICE WALL, not an attacker: once the line is developing (Dreepy/Drakloak/Dragapult in play)
    #    OR it's turn >=3, NEVER attack with Budew — force END (or let bare_agent develop) instead.
    #    This frees those turns for evolve/attach/develop so Dragapult deploys on mogja_j's T3 timeline.
    if active_is_budew and (line_developing or turn >= 3):
        for i, o in enumerate(opts):
            if o.type == OptionType.ATTACK and getattr(o, "attackId", None) == ITCHY_POLLEN:
                _log(turn, "suppress Budew attack", "Budew is a wall not an attacker; line developing → END/develop instead of wasting a turn on 10dmg")
                # pick END if available (cleanest); else fall through to bare_agent's non-attack choice
                for j, o2 in enumerate(opts):
                    if o2.type == OptionType.END:
                        return [j] if sel.minCount <= 1 else [j]
                break
