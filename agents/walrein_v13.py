            ...
            for i, o in enumerate(opts):
                if o.type == OptionType.PLAY:
                    ...
                    if c is not None and c.id == 1231:
                        return [i] if select.minCount <= 1 else [i]
        # v13 BUDEW ITEM-LOCK: if Budew (235) is active early (turn<=3), prefer Itchy Pollen
        # (item-lock: opp can't play Items next turn) -> disrupts ramp-deck setup (hydrapple/draga/bolt).
        _my_active_id = (my_state.active[0].id if (my_state.active and my_state.active[0]) else None)
        if _my_active_id == 235 and (state.turn or 0) <= 3:
            opts = select.option or []
            for i, o in enumerate(opts):
