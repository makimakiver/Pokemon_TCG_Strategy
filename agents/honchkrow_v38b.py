...
my_deck = ([463] * 4 + [891] * 4 + [473] * 1 + [474] * 1 + [414] * 1 +
           [434] * 1 +                       # Team Rocket's Mimikyu (60hp Fighting-RESISTANT wall)
           [1220] * 4 + [1216] * 4 + [1218] * 4 + [1219] * 4 + [1217] * 4 +
           [1225] * 1 +                       # Hilda (SUPPORTER: tutor Evolution + Energy)
           [1122] * 4 + [1134] * 4 + [1152] * 3 + [1077] * 2 +   # Pokegear 3.0, Transceiver, Poke Pad, Roto-Stick
           [1097] * 1 + [1109] * 1 +         # Night Stretcher, Miracle Headset
           [1257] * 4 +                       # Team Rocket's Factory
           [15] * 4 + [17] * 4)               # TR Energy, Ignition
# deck = lean consistency/density build (12 Pokemon / 40 trainer / 8 energy): max draw+search
# density to fuel Rocket Feathers/R-Command + the mill/stall engine. Mimikyu = Fighting-resistant
# wall body; Hilda = Evolution+Energy tutor. (Night Stretcher kept for recovery.)
...
MIMIKYU = _card_named("Team Rocket's Mimikyu")         # v37: 60hp Fighting-resistant wall body
            ...
            if card is not None:
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    ...
                    if card.id == MURKROW:
                        ...
                        # v38b: vs Fighting, Porygon (60hp Fighting-WEAK) is a free OHKO ->
                        # suppress from passive bench; only bench if board is dangerously thin.
                        score = 90 if not opp_fighting else 20
                    elif card.id == MIMIKYU:
                        # v37/v38b: Mimikyu is 60hp Fighting-RESISTANT + free retreat, BUT
                        # Aura Jab 130-30=100 still OHKOs it. So vs Fighting it's a ONE-TURN
                        # wall at best, not a safe body. Demote vs Fighting (death trap that
                        # gifts tempo); promote vs non-Fighting (it banks a hit safely there).
                        score = 60 if opp_fighting else 105
                    elif card.id == ARTICUNO:
                        # v38b: vs Fighting, Articuno (120hp Fighting-resistant) is a REAL wall.
                        score = 30 if not opp_fighting else 120
                    elif card.id in BASIC_MONS:
                        score = 80
                    else:
                        score = 50
