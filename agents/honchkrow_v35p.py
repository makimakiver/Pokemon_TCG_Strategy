...
# v35p D1-ENERGY instrumentation counters (module-level; reset per game, read by audit).
# These let us answer the planner's demand: HOW MANY times did we route energy to Porygon2,
# how many times did Door 1 actually fire, and did conversions cost games elsewhere.
_stats = {"d1_energy_attaches": 0,   # times we routed an attachment to Porygon2 for the D1 line
          "d1_porygon2_developed": 0,  # games where a Porygon2 reached >=3 energy at least once
          "d1_kills_fired": 0,          # games where R-Command actually took a KO
          "games": 0}


def _reset_game_stats(state_turn):
    # reset per-game accumulators on turn 0 (game start)
    if state_turn == 0:
        _stats["d1_porygon2_developed"] = 0
        _stats["d1_kills_fired"] = 0
...
def agent(obs_dict):
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                if mine.id == PORYGON2 and not kos and i != 0:
                    score -= 150
                # v35p D1-ENERGY: once we've INVESTED 3 energy into Porygon2 for the closer line,
                # a LETHAL R-Command is the whole point — let it win the plan decision (it doesn't
                # burn hand supporters, the planner's asymmetry win). Big bonus when it KOs.
                if mine.id == PORYGON2 and is_rc and kos and len(mine.energies) >= 3:
                    score += 800
                    if op_prize_left <= prize_count(opp):
                        score = 60000   # game-winning R-Command snipe
    ...
    def energy_score(pokemon, is_active, is_ignition):
        ...
        if len(pokemon.energies) >= need:
            score -= 220
        # v35p D1-ENERGY (planner spec): at prize>=3 vs Fighting, if R-Command discard fuel
        # is already lethal (rc_fuel >= ceil(mega_hp/20)) AND a benched Porygon2 has <3 energy,
        # divert THIS energy attachment to Porygon2. Energy and hand-supporters are DIFFERENT
        # resources (the planner's asymmetry point) — this does NOT starve the mill engine.
        # Terminal pivot only (never pre-commit; only when the kill window is confirmed).
        if pokemon.id == PORYGON2 and not is_active:
            op_active_hp = (op_active.hp if op_active is not None else 0)
            fuel_lethal = rc_fuel * RC_PER_CARD >= op_active_hp and op_active_hp > 0
            if opp_fighting and (6 - op_prize_left) >= 3 and len(pokemon.energies) < 3 and fuel_lethal:
                score = 9000 + len(pokemon.energies) * 50   # override: build the D1 closer
        return score
            ...
            if card is not None and card.id in TOOLS:
                ...
                if plan.needs_energy:
                    if plan.attacker == 0 and o.inPlayArea == AreaType.ACTIVE:
                        ...
                        score += 300
                # v35p D1-ENERGY: count when we actually route energy to a benched Porygon2
                # that's short of 3 energy, with the kill window confirmed.
                if (pokemon is not None and pokemon.id == PORYGON2
                        and o.inPlayArea != AreaType.ACTIVE and len(pokemon.energies) < 3
                        and opp_fighting and (6 - op_prize_left) >= 3):
                    op_active_hp = (op_active.hp if op_active is not None else 0)
                    if rc_fuel * RC_PER_CARD >= op_active_hp and op_active_hp > 0:
                        score = max(score, 9200)   # win the attach decision
                        if len(pokemon.energies) < 3:
                            _stats["d1_energy_attaches"] += 1
                            if len(pokemon.energies) == 2:   # this attach completes 3 energy
                                _stats["d1_porygon2_developed"] = 1
    ...
    # v35p D1 instrumentation: if the chosen action this turn IS a Porygon2 R-Command attack
    # that we marked lethal in the plan, count it as a D1 kill attempt.
    if _stats["d1_porygon2_developed"] and plan.attack_id == R_COMMAND and plan.remain_hp <= 0:
        _stats["d1_kills_fired"] += 1
    return chosen
