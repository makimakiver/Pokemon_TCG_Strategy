"""Walrein/Spheal control — v1 = bare_agent + LOCK-PRIORITY engine (Frigid Fangs is the win condition).
...
def agent(obs_dict):
        ...
        for i, mine in enumerate(my_cards):
            ...
            for aid in (cd.attacks or []):
                ...
                for j, opp in enumerate(op_cards):
                    ...
                    # v1 L1 LOCK-PRIORITY: Walrein 'Frigid Fangs' [W] 60dmg LOCKS the opp
                    # (their <=2-energy Pokemon can't attack next turn). The lock IS the win
                    # condition — score Fangs by LOCK VALUE, not its 60 printed damage.
                    is_fangs = (mine.id == WALREIN and aid == FRIGID_FANGS)
                    is_megaton = (mine.id == WALREIN and aid == MEGATON_FALL)
                    if is_fangs:
                        # does the opp active qualify for the lock (<=2 energy)?
                        op_en = len(opp.energies) if hasattr(opp, 'energies') else 0
                        lock_eligible = op_en <= 2
                        # is the opp threatening (their active can damage us)?
                        my_active = my_state.active[0] if my_state.active else None
                        my_hp = my_active.hp if my_active else 0
                        opp_threat = False
                        op_cd = card_table.get(opp.id)
                        if op_cd:
                            for oa in (op_cd.attacks or []):
                                oatk = attack_table.get(oa)
                                if oatk and (oatk.damage or 0) >= my_hp * 0.5:
                                    opp_threat = True; break
                        if lock_eligible:
                            # LOCK MODE: Fangs removes the opp's next turn -> near-infinite value
                            # unless we have a game-winning KO available this turn anyway.
                            base_lock = 3000
                            if opp_threat:
                                base_lock = 8000       # prevents a KO we'd otherwise eat
                            # if Megaton would KO here, that's still better than a lock (win-now)
                            megaton_kos = False
                            if MEGATON_FALL and mine.id == WALREIN:
                                ma = attack_table.get(MEGATON_FALL)
                                if ma and opp.hp <= (ma.damage or 0):
                                    megaton_kos = True
                            if not (megaton_kos and op_prize_left <= prize_count(opp)):
                                score = max(score, base_lock)
                        # lock-irrelevant case (opp has >=3 energy): Fangs is just 60 chip,
                        # handled by normal scoring (score *= dmg/hp) — fall through
                    if i == 0:
                        score += 200
    ...
    def energy_score(pokemon, is_active):
        ...
        if pokemon.id in ATTACKERS:
            ...
            score += 150
        if pokemon.id == WALREIN:                         # v1: feed the Walrein lock attacker first
            score += 200
