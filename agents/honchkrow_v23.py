"""Team Rocket's Honchkrow — research-driven heuristic pilot (v23 = v14 + Fighting-gated Porygon-suppression).

V23 CHANGE vs v14 (data-driven, see discussion.log + tools/loss_probe.py):
  vs a Fighting deck (opp_fighting, e.g. Mega Lucario ex) the Porygon/Porygon2 line is
  Fighting-WEAK and only 90 HP, so every benched/promoted Porygon is a free OHKO prize
  that accelerates the opponent's race and enables the board sweep that is ~50% of our
  losses. So vs Fighting we SUPPRESS the Porygon line as passive bodies:
    P1  never OPEN Porygon as the active (setup-active).
    P2  never PREFER Porygon on the bench — demote to a fallback body only; lean on the
        Honchkrow line (Murkrow/Honchkrow, Lightning-weak = safe vs Fighting) + Articuno
        as a passive wall.
    P3  never PROMOTE Porygon/Porygon2 to active (retreat/switch) unless it KOs now.
    P4  never DEVELOP Porygon->Porygon2 on the bench unless R Command is live (rc_fuel
        high enough to matter) — don't sit a 90HP Fighting-weak prize in play.
  Inert in non-Fighting matchups (guard: abomasnow_cal).
...
def agent(obs_dict):
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
                ...
                if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
                    if o.playerIndex == my_index:
                        ...
                        if card.id == HONCHKROW:
                            ...
                            score += 10
                        # P3: vs Fighting, never promote the Fighting-weak Porygon line.
                        if opp_fighting and card.id in (PORYGON, PORYGON2):
                            score -= 300
                        if card.id == ARTICUNO:
                    ...
                    elif card.id == PORYGON and opp_fighting:
                        score = 10              # P1: never open Porygon active vs Fighting (free OHKO)
                    ...
                    if card.id == MURKROW:
                        ...
                        # P2: vs Fighting, Porygon (90HP Fighting-weak) is a free prize —
                        # demote to a fallback body so safe Honchkrow-line/Articuno rank above.
                        score = 90 if not opp_fighting else 15
                    elif card.id == ARTICUNO:
                        score = 30 if not opp_fighting else 45   # safe passive wall vs Fighting
                    elif card.id in BASIC_MONS:
                        score = 80
                    else:
                        score = 50
            ...
            if card is not None:
                ...
                if ct == CardType.POKEMON:
                    ...
                    if cid in BASIC_MONS and n_bodies >= 5:
                        score = 30
                    # P2 (play): vs Fighting, don't develop Porygon (90HP Fighting-weak) onto
                    # the bench as a passive prize once we already have >=2 bodies; only play
                    # it when the board is dangerously thin.
                    if opp_fighting and cid == PORYGON and n_bodies >= 2:
                        score = 25
            ...
            # P4: vs Fighting, evolving Porygon->Porygon2 parks a 90HP Fighting-weak prize
            # in play. Only do it when R Command is live (rc_fuel high enough to be a real
            # hit) — otherwise keep Porygon as a hidden basic and don't expose the evo.
            if opp_fighting and pokemon is not None and pokemon.id == PORYGON:
                score = 9000 if rc_fuel >= 8 else 40
