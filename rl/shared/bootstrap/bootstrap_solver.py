...
checkpoint that ``rl.sgs.train_solver(init_policy=...)`` continues RL from, and that
...
Run:
  docker run --rm --platform=linux/amd64 -v "$PWD":/app -w /app cabt-rl \
    -m rl.shared.bootstrap.bootstrap_solver --teacher-games 40 --epochs 3 --eval-games 40
"""
...
from rl.shared.policy import PointerPolicy, save as save_policy
from rl.shared.bootstrap.sft import collect_traces, train_sft
from rl.shared.bootstrap.distill_claude import head_to_head   # net (greedy) vs a module pilot, seats swapped
...
def main():
    ...
    print("=" * 70)
    print("Next: rl.sgs.train_solver.train(init_policy=rl.shared.policy.load('%s'))" % args.out)
