...
checkpoint that ``rl.train_solver(init_policy=...)`` continues RL from, and that
...
Run:
  docker run --rm --platform=linux/amd64 -v "$PWD":/app -w /app cabt-rl \
    -m rl.bootstrap_solver --teacher-games 40 --epochs 3 --eval-games 40
"""
...
def main():
    ...
    print("=" * 70)
    print("Next: rl.train_solver.train(init_policy=rl.policy.load('%s'))" % args.out)
