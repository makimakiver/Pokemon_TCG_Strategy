...
``rl.training.curriculum.conjecturer.author.LLMConjecturer`` wrote during an ``outer_loop`` generation:
...
Run on the GPU pod (needs rl/requirements-llm.txt):
  docker run --rm --gpus all -v "$PWD":/app -w /app cabt-rl-llm \
    -m rl.training.curriculum.conjecturer.cispo_train --epochs 2
"""
...
def train_conjecturer_cispo(buffer_path: str | None = None, epochs: int | None = None):
    ...
    from rl.training.curriculum.conjecturer.author import _SYSTEM   # same system prompt the buffer rows were sampled under
