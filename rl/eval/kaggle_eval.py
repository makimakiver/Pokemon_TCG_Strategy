...
Run inside the cabt-rl image (cg/libcg.so is Linux x86-64 only):
    ...
    docker run --rm --platform=linux/amd64 -v "$PWD":/app -w /app \
      --entrypoint python cabt-rl -m rl.kaggle_eval

    # any checkpoint + game count + search budget
    docker run --rm --platform=linux/amd64 -v "$PWD":/app -w /app \
      -e RL_EVAL_GAMES=50 -e RL_SEARCH_COUNT=10 \
      --entrypoint python cabt-rl -m rl.kaggle_eval out/model3.pth
"""
import os
import sys
...
def main():
    ...
    if not os.path.exists(ckpt):
        raise FileNotFoundError(
            f"checkpoint not found: {ckpt} — run rl.kaggle_mcts first to produce out/model*.pth")
