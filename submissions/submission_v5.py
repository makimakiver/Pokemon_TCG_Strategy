"""COMPETITION SUBMISSION — Team Rocket's Honchkrow / Porygon2 (self-contained).

This is the shippable `(deck + agent)` pair derived from `agents/main_v5.py` (the
RL Solver's scripted pilot), inlined so it depends ONLY on the bundled `cg` engine
— no `rl/`, no `data/`, no file reads, no external API calls. The 60-card deck is
hard-coded below and also shipped as `deck.csv`.

Deck = Team Rocket's Honchkrow, limitlesstcg 26267 (the Ultra-Ball-flex variant).
...
# ---- Deck (inlined, self-contained) == limitlesstcg 26267, Ultra-Ball flex ----
#  4 Murkrow(463) 3 Honchkrow(891) | 2 Porygon(473) 1 Porygon2(474) | 2 Articuno(414)
#  4 Proton(1220) 4 Ariana(1216) 4 Giovanni(1218) 4 Petrel(1219) 4 Archer(1217)
#  4 Poke Pad(1152) 4 Roto-Stick(1077) 4 Transceiver(1134) 3 Night Stretcher(1097)
#  1 Miracle Headset(1109, ACE SPEC) 1 Ultra Ball(1121) 3 Factory(1257)
#  4 Team Rocket's Energy(15) 4 Ignition Energy(17)
